...
**Files:** `WebServer.swift` (`FeedRelayProxyDelegate`), `AppState.swift` (`startFeedLocalRelay`)

**Symptom**: watching another Mac's in-progress recording via FEED (cross-machine, `/auto/v<channel>`) would freeze for anywhere from several seconds to 90+ seconds at a time, resolve, and repeat — sometimes several times inside a few minutes — while the source relay's own log showed continuous, gap-free byte delivery the entire time. Reproduced identically in stock, unmodified VLC.app (no dependency on this app's own code at all), confirming it wasn't specific to how this app drives libvlc.

**Root cause: delivery *cadence*, not the network, the data, or VLC's settings.** A viewer's own network read of the remote FEED lands in bursts — whatever pattern the real network hop happens to produce — and this app's growing-file relay mechanism (`streamGrowingFile`/`pumpGrowingFile`, the same one Watch Now uses flawlessly) just serves "whatever's newly available since the last poll," faithfully reproducing that bursty pattern to VLC. VLC's own PCR/clock-sync recovery logic can't tolerate that irregular cadence the way it tolerates a real tuner's naturally steady delivery — confirmed via thread-level `sample` captures showing VLC's entire demux pipeline going idle mid-stall even while bytes kept arriving on the wire.

**Decisive proof**: the exact same bytes, read two ways. Through the growing-file relay: repeated stalls with the signature `ES_OUT_SET_(GROUP_)PCR is called too late` → `no reference clock` → full rebuffer. Read as a plain static file snapshot instead (VLC's file-length-at-open-time behavior conveniently isolates this): zero stalls, played perfectly. Same data, same VLC, same machine — only the delivery pattern differed. This ruled out corrupted/discontinuous broadcast content (also independently confirmed via a purpose-built TS-level PCR analyzer, `tools/feed_diagnostics/`, across 13/13 real captured stalls) and pinned the cause squarely on cadence.

**What didn't work** (each live-tested and reverted, kept brief here so a similar idea isn't retried blind):
- A live-edge cushion (deliberately staying a little behind the true live edge) and various chunk-size tunings — no effect either way, or made it worse.
- VLC's `--no-ts-trust-pcr` and `--stream-filter=record` flags — both broke VLC's own startup/buffering ramp entirely (never reached first frame), a worse failure than the original bug.
- A guessed-bitrate delivery-pacing timer on the *source* side — this file's growth was already correctly real-time-paced there (curl reading directly from the tuner), so an extra guessed delay just added jitter instead of removing it.
- A disk-backed client-side relay (a puller process writing the remote bytes to a local temp file, re-served over loopback via the same growing-file mechanism) — reproduced the identical stall even with VLC's own socket now provably loopback-only, which is what actually proved the growing-file *mechanism* itself wasn't the problem: the real network hop's bursty arrival pattern was surviving the disk round-trip intact, because nothing paced it before or after.

**The actual fix**: pace delivery to VLC at a steady real-time rate, smoothing out whatever bursty pattern the network hop produces, instead of tuning buffer sizes or VLC flags. Implemented as `WebServer.FeedRelayProxyDelegate` — an in-memory, in-process proxy on the viewer Mac: one `URLSession` connection to the remote Mac's FEED URL, buffering incoming bytes and draining them to VLC using an EWMA of the observed byte-arrival rate, holding chunks back just long enough that cumulative delivery tracks real time (plus 0.5s of lookahead slack) — except while genuinely draining a backlog (e.g. a "catch up to live" reconnect), which is allowed to run at full speed since the static-file test above already showed a backlog drains cleanly regardless of speed. No disk involved at all. Confirmed via live A/B: the app's own relay (through this proxy) plays clean; plain VLC.app against the same raw remote URL, unpaced, still stalls — same content, same instant, only the code path differs.

**Resolving commits**: `96c72eb` (an intermediate disk-backed pacer, superseded same day), `f5af27b` (the in-memory proxy that actually shipped), `61dd617` (docs/live A/B confirmation)
...
## RESOLVED — A tuner dropping mid-recording cut off an already-watching Watch Now/FEED viewer immediately, even with real content still on disk

**Files:** `AppState.swift`, `WebServer.swift`

**Root cause**: live-reported 2026-09-13 — "if a tuner drops offline before a watch now or feed user is complete with watching the show, we shouldnt tear down the video, if we have this on disk." `idleLoop`'s curl-died detection (Pass 2) called `teardownRecordingState`, which unconditionally flipped `show_recording` false and force-killed any active transcode session. `WebServer.handleWatchRecording`/`handleVirtualTunerStream` both gated *every* request — including a scrub-bar seek, a pause/resume, or a player's own reconnect from an already-watching viewer — on `show.show_recording == true`; once that flipped false, any such reconnect was rejected outright regardless of how much valid content already existed on disk. This gate itself is deliberate, not a bug: `handleWatchRecording`'s own comment explains `show_recording_path` is never cleared, so without it a finished recording could be replayed indefinitely to any LAN client (no auth beyond subnet matching) — any fix had to preserve that.

**Fix**: added a bounded (120s) per-show "abnormal-stop grace window" (`ShowRuntimeState.abnormalStopGraceUntil`), set *only* by the abnormal (tuner/curl-died) stop path — never by natural, manual, skip, or delete stops, so the anti-replay gate is never actually weakened, just given a narrow, wall-clock-bounded exception for the one case where the stop wasn't a deliberate choice. `AppState.recordingIsWatchable(_:)` is the new single gate both WebServer routes check instead of raw `show_recording`. A new `idleLoop` pass (`expireAbnormalStopGraceWindows`) enforces the bound once nothing resumes (force-killing any still-deferred transcode session at that point). `updateVirtualTunerPresence` now also counts a grace-covered show toward whether the FEED virtual tuner stays advertised — without this, a remote viewer's reconnect would have nothing to hit even with the WebServer-side gate fixed, since the virtual tuner itself would already be torn down. Explicit delete/skip (`deleteShow`, `skipRecording`) clear the grace and force-kill any deferred transcode session immediately, regardless of which stop path preceded them — an explicit user action always wins over the grace.

**Resolving commit**: `99c8a2c`

## RESOLVED — "Add Show…" could take several seconds to open, most noticeably right after launch

**Files:** `AppState.swift`, `WebServer.swift`, `Views/AddShowView.swift`, `docs/WebServer.md`, `docs/AddShowView.md`

**Root cause**: live-reported 2026-09-13 — "When i click Add show in the menu, this is slow." Add Show's guide step embeds this app's own LAN web guide in a `WKWebView` pointed at `GET /`. That route normally serves a pre-warmed page cache (`cachedHTML`/`cachedHTMLGzip`), but if hit before the *first* `prebuildPageHTML()` pass had ever run — e.g. right after launch, before the cloud `guide.php` fetch completed — it fell back to a live, synchronous, `@MainActor`-blocking `buildGuideGridHTML()` pass, already documented (`docs/WebServer.md`) as a 2-4 second full main-thread freeze on cold cache.
