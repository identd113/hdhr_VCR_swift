...
**Proposed mitigation — show a Dock icon (at least) until Local Network permission is confirmed granted, then let the user hide it again.** Root theory across this whole investigation has been that `LSUIElement` (no Dock icon, no main window) is why the real consent dialog either never fires or fires somewhere the user never notices — there's no Dock bounce, no window to anchor it to, nothing drawing attention for a pure menu-bar-only process. A background app with no on-screen presence is exactly the case Apple's own docs warn is unreliable for system permission prompts.

Sketch of the fix, not yet implemented:
- Stop setting `LSUIElement = true` unconditionally in Info.plist; instead call `NSApp.setActivationPolicy(_:)` at runtime, which Apple explicitly supports switching at any time (`.regular` = Dock icon + app switcher, `.accessory` = today's menu-bar-only behavior). Info.plist's `LSUIElement` only controls the *very first* instant of process launch before any app code has run, so getting a Dock icon to exist at prompt time means Info.plist must *not* start the process as accessory — the code has to explicitly downgrade to `.accessory` afterward instead.
- On first launch (or any launch where Local Network access hasn't yet succeeded), start as `.regular` so there's a Dock icon and the app can become key/frontmost — giving the system's permission dialog a normal foreground app to attach to and the user an obvious reason to look. Once `fetchAllLineups`/`ensureLineupLoaded` reports a real success (`[Lineup] ... loaded N channels` in today's fixed logging — see above), or the user has otherwise dismissed/resolved the prompt, switch to `.accessory` to return to today's menu-bar-only behavior.
- Add a **Settings → Advanced → "Show Dock Icon"** toggle (persisted in `AppConfig`, mirroring existing boolean settings like `Series_subfolder_enabled`) so a user who wants the Dock icon permanently visible (or permanently hidden, accepting the permission-prompt risk) can override the auto-hide behavior explicitly rather than only ever getting the launch-time heuristic.
- Worth confirming whether `.accessory` apps can even *receive* the Local Network prompt UI event while backgrounded, vs. simply never being offered it — if the OS literally never dispatches the dialog to an accessory-policy process, this isn't just "easier to see," it may be the actual fix, not just a UX nicety.

**Key files**: `AppState.swift` → `fetchAllLineups`, `toggleFavorite` (both already correct). `HDHRManager.swift` → `fetchLineup` (unaffected, no in-function change needed since its caller handles the error), `fetchDeviceInfo`'s two callers (instrumented today), `setFavorite` (verified already correct). Dock-icon mitigation (not started): `hdhrVCRplus.app/Contents/Info.plist` (`LSUIElement`), `hdhr_VCRApp.swift` (entry point — where `NSApp.setActivationPolicy` would need to be called), `SettingsView.swift` + `Models.swift`'s `AppConfig` (new toggle).
