...
Added 2026-09-06: the menu bar blue light (`AppState.hasAvailableRemoteFeed`/`.feedAvailable` in `statusLightCandidates`) lights up whenever *any* remote hdhrVCRplus instance's Recording FEED relay is discovered with a show attached — regardless of whether anyone is actually watching it from this Mac or anywhere else. Flagged live 2026-09-06, deferred: arguably this status should only matter (and only light up) once a real viewer is connected to that relay, not just because it exists and *could* be watched — otherwise it's less "something needs your attention" and more "a relay happens to be up," which is a much weaker signal. Not scoped: would need the discovering side to know the relay's own current viewer count (the source Mac already tracks this internally — `relayRawViewerCount`/`VLCBridge.transcodeViewerCount`, surfaced today only as `MenuContent`'s own "FEED: N watching" header row on the *source* Mac), or deciding the light should instead reflect "am *I* currently watching this," which is trivially already known locally (`VLCBridge.shared.currentURL` matching a remote relay entry's own URL) but changes what the light actually means.

**Fully unblocked, 2026-09-13**: the missing piece identified above — a raw-passthrough viewer count published the same way `HdhrVCRplusTranscodeViewers` already was — now exists. `AppState.relayRawViewerCounts` (was a single machine-wide `Int`, now per-`show_id`) is published as `HdhrVCRplusRawViewers` in `/lineup.json` (omitted unless >0, same convention), and both this and the existing transcode-viewer count now trigger a fresh UDP announce on every connect/disconnect (`refreshVirtualTunerAnnounceIfActive()`) instead of only on the relay's own appear/disappear/`TunerCount` change — and the *receiving* side's `onFeedAnnounce` handler now re-fetches an already-known relay's `/lineup.json` immediately instead of waiting for the hourly `refreshGuides()` pass. See `docs/VirtualTunerService.md`'s "Viewer-count-triggered announces" and `HdhrVCRplusRawViewers` entries for the full mechanism. **Still not done**: `hasAvailableRemoteFeed`/`statusLightCandidates` itself still only checks `virtualRelayShowTitle != nil` — the data needed to make it check real viewer count (raw + transcode, summed, `> 0`) is now available and fresh, but the light's own logic hasn't been changed to use it. That's the one remaining step, and a real product decision (this framing) vs. "am I currently watching this" (the alternative framing from the original entry) is still open either way.
...
2. Live testing so far covers short-lived sessions (~15-20s windows). Multiple concurrent viewers of the same show sharing one `TranscodeSession` correctly (the reference-counting path, including across *different* requested profiles — sharing is now keyed by `showId` alone, not `showId`+profile, per a 2026-09-03 explicit user request) has been live-verified against the real deployed app. Not yet verified: a long-running session's resource behavior over a full-length recording rather than a short test window.
   **2+ concurrent viewers under real load — tested 2026-09-13, not just theoretical.** Sharing itself works correctly (confirmed via the real `refs=2` join log line), but 2 concurrent viewers plus a live recording pushed a real M2 Mac mini to 100-220% CPU with recurring `late buffer for mux input` warnings, despite three same-day fix attempts (output pacing, a reverted input-lag idea, input pacing) — see `ISSUES.md`'s "Accepted" entry for the full trail. Scoped down: FEED transcode is being treated as reliable for one in-app viewer at a time for now, not a general multi-viewer mechanism.
...
Flagged 2026-09-08, originally framed around the live-edge cushion added the same day — that cushion (and its own "recording finished, drain the remaining tail before closing" special case) was removed entirely 2026-09-13, proven a dead end for the separate VLC stall investigation (see `issues_resolved.md`), so that framing no longer applies. What's left is the concern the cushion angle was a side note to in the first place: `pumpGrowingFile`/`handleGrowingFileChunk`'s normal (always-was, cushion-independent) behavior already drains any real backlog before a genuinely-empty read closes the stream — a viewer with unread bytes still on disk keeps getting served them regardless of `show_recording`'s own value, by construction (an empty read only happens once `bytesSent` has actually caught up to true EOF). Not yet verified for the case the user actually described: a viewer who has drifted **far** behind the live edge — e.g. a full minute, via VLC's own client-side jitter-buffer growth (see the "close to 1:1" FEED discussion the same day) or a deliberate seek — at the moment the source show's recording ends. Need to confirm live: that no *other* code path (a stop-recording handler, a connection-cleanup pass) force-closes an in-progress FEED viewer connection the instant `show_recording` flips false, independent of `pumpGrowingFile`'s own drain logic — if one exists, it would cut off a deeply-behind viewer mid-show even though the read-loop itself would have kept draining correctly on its own.

---

### ~~Other instances' "Recording on Another Mac" menu should update promptly once a FEED session naturally winds down~~ — done 2026-09-12

Flagged 2026-09-08. A partial mechanism already existed (`VirtualTunerService.stop()`, added 2026-09-06, `3e45b82`) — one last `broadcastAnnounce()` before clearing state, nudging a listening instance's `onFeedAnnounce` into an immediate `probeForNewDevices()` call instead of waiting for the next ~10s idle-loop tick. Missed on first pass writing this entry (corrected same day after checking for prior art properly): that alone only sped up *when* the next probe ran, not the **3 consecutive missed probes** (`AppState.swift`'s `missedProbes`/`missed == 3`) `probeForNewDevices()` still required before actually marking the device unavailable — so the real-world result was "menu updates ~2 idle-loop cycles sooner," not instant removal.

**Actual fix, 2026-09-12**: `broadcastAnnounce(goodbye:)` — `stop()`'s final call passes `goodbye: true`, which sends `tunerCount: 0`. `AppState`'s `onFeedAnnounce` closure pre-sets that device's `missedProbes` to `max(current, 2)` when this fires, *before* calling `probeForNewDevices()` — so that very probe (which legitimately won't find a relay that just announced its own stop) hits the existing `missed == 3` branch and marks it unavailable on the same cycle, not ~2 ticks later. `max(...)` rather than a flat assignment so this can never lower an already-higher count from real prior misses. (Originally implemented via a non-standard `0xF0` TLV and a `Bool` on `onFeedAnnounce` — simplified 2026-09-13 to just checking the announced `TunerCount == 0` directly, since a live relay never legitimately advertises a zero count otherwise; see `issues_resolved.md`.) See `docs/VirtualTunerService.md`'s "Unsolicited FEED announces"/"Wire protocol" sections for the full writeup; unit tests in `VirtualTunerServiceTests.swift` cover the `TunerCount` round-trip. **Not yet live-tested** against a real cross-machine relay stop — build/tests pass, not deployed yet.
...
**A first concrete consumer landed 2026-09-12, via a narrower route than the fuller feature above**: `VLCPlayerView`'s poster overlay needed real show identity during a FEED session's buffering wait (`feedAutoPlayMinDelay`) and had nothing — `currentGuideEntry` is exactly the `guideByDevice[relayId]`-backed lookup this TODO's fuller feature would eventually populate, so it's still always `nil` for a relay device. Rather than building the full `guideByDevice[relayId]` population machinery for this one consumer, added four more `/lineup.json` extras (`HdhrVCRplusEpisodeTitle`/`EpisodeNumber`/`Synopsis`/`ImageURL`, mirroring `AppState.DiscordEpisodeSnapshot`) and read them directly via `currentFeedEntry`, bypassing `GuideEntry`/`guideByDevice` entirely — see `docs/VirtualTunerService.md`'s `/lineup.json` section and `docs/VLCPlayerView.md`'s matching note. The fuller feature above is still not done and still has no other consumer; this only solves the one concrete UI need that existed.
...
- **VLC pointer** — ✅ shipped 2026-09-12: `FirstRunWizardView`'s new `Step.vlcRequired` screen explains the VLC dependency (live install-status check, "Check Again", and a Homebrew-detected copy-command-and-open-Terminal button or a direct-download link otherwise) — see `docs/FirstRunWizardView.md`. Same "applies to every install" note as Local Network permission above; this landed alongside removing the unreliable "Open in VLC" feature entirely (`CHANGELOG.md`'s "Unreleased" section).
