...
**Concrete mechanism proposed 2026-09-12, reusing what already exists rather than inventing something new**: `VirtualTunerService` already has exactly half of this — `broadcastAnnounce()` sends an unsolicited UDP broadcast the moment a relay starts advertising (`isAdvertising` flips true), using the *same* wire format/port (65001) real HDHomeRun discovery uses, and the receiving side (`onFeedAnnounce`) already self-filters on `DeviceID` so it can't be confused with a real device's own discovery reply (see that file's own doc comments on `lastBroadcastDeviceID`'s loopback-guard). A symmetric "goodbye" broadcast — same socket, same targets (subnet-directed + `INADDR_BROADCAST` fallback), fired once from wherever the relay's teardown path already lives (the same place `isAdvertising` flips back to `false`) — would close the loop without adding any new protocol surface: no new port, no new packet shape, just the existing announce sent at relay-end instead of only at relay-start. The receiving side would need a way to tell "new relay" from "relay ending" apart in one wire message; the safest option is probably a payload byte/sentinel field rather than overloading `DeviceID` (e.g. `DeviceID=0` colliding with something else) — needs a look at the real HDHomeRun discovery packet's own field layout to find an unused byte, or accepting the small oddity of using this app's own non-standard extension fields (already precedent: `HdhrVCRplusVirtualRelay`/`HdhrVCRplusShowTitle` in `/discover.json`/`/lineup.json`) if the raw UDP discovery-reply packet itself has no spare room. On the consuming side, `AppState` would remove that device from `remoteRelayEntries`/close its menu entry immediately on receipt, rather than waiting for `probeForNewDevices()`'s next poll to notice it went stale. Not yet scoped further — needs the packet-format decision above before implementation.
