...
**Partially unblocked, 2026-09-04's transcode-sharing work (predates this TODO but relevant)**: `HdhrVCRplusTranscodeViewers` (`VirtualTunerService.transcodeViewersKey`, decoded onto `LineupEntry.virtualRelayTranscodeViewers`) already exists in `/lineup.json` and is already readable by a discovering instance — it just isn't wired into `hasAvailableRemoteFeed`/`statusLightCandidates` today. But it only covers *transcoded* viewers (`VLCBridge.shared.transcodeViewerCount(showId:)`), not raw-passthrough ones — and `watchRemoteRelay` always requests raw (`docs/VirtualTunerService.md`'s "Remote watch menu entry"), so the common case (this app's own instances watching each other) would still be invisible to it. Wiring the light to this field alone would be an incomplete fix, not a full one; a real fix still needs an analogous raw-viewer-count field published the same way, not a genuinely new mechanism from scratch the way this entry originally framed it.
...
**Actual fix, 2026-09-12**: `broadcastAnnounce(goodbye:)` — `stop()`'s final call now passes `goodbye: true`, which sends `tunerCount: 0` plus a new non-standard `0xF0` TLV (`buildDiscoverReply(isGoodbye:)`/`isGoodbye(fromReplyPacket:)`, `VirtualTunerService.swift`) that a real client just ignores like any other unknown TLV. `onFeedAnnounce`'s signature widened to `(String, Bool)`; `AppState`'s closure pre-sets that device's `missedProbes` to `max(current, 2)` when the bool is true, *before* calling `probeForNewDevices()` — so that very probe (which legitimately won't find a relay that just announced its own stop) hits the existing `missed == 3` branch and marks it unavailable on the same cycle, not ~2 ticks later. `max(...)` rather than a flat assignment so this can never lower an already-higher count from real prior misses. See `docs/VirtualTunerService.md`'s "Unsolicited FEED announces"/"Wire protocol" sections for the full writeup; unit tests in `VirtualTunerServiceTests.swift` cover the new TLV round-trip. **Not yet live-tested** against a real cross-machine relay stop — build/tests pass, not deployed yet.
