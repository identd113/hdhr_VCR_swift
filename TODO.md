...
**This already has a concrete, confirmed bug backing it, not just a nice-to-have.** `docs/VirtualTunerService.md`'s own "Known limitation" section documents `guideByDevice[relayId]` as intentionally never populated (`AppState.recordableDevices` is the accessor essentially every guide-fetch call site filters through specifically to exclude relay devices) — but `probeForNewDevices()` (`AppState.swift:1157-1162`) misses it: `newDevices = (found ?? []).filter { !existingIDs.contains(...) }` has no `isVirtualRelay` filter, so a **newly-discovered** relay device flows straight into `guideStore.loadAll(devices: newDevices, ...)` — the one guide-fetch call site of the several in `AppState.swift` that isn't scoped to `recordableDevices` (contrast `fetchAllGuides()`/`performFetchAllGuides()`, both correctly `recordableDevices`-scoped). Live-caught 2026-09-06 in exactly this shape: `[FEED04BE] load() called — DeviceAuth:nil LocalIP:'10.0.2.100' hours:24` immediately followed by a `NETWORK ERROR` (`A TLS error caused the secure connection to fail`, failing URL `https://10.0.2.100/guide.json?...`) — a real, if harmless-in-practice, wasted request against a same-LAN address that has no such endpoint (this app's own `WebServer` is HTTP-only and doesn't implement `/guide.json` at all), happening the instant another instance's FEED is first discovered.

**Quick, narrowly-scoped fix for that one call site** (not yet applied — deferred alongside the bigger feature below since they're the same root cause): filter `newDevices` to `recordableDevices`-equivalent (`!$0.isVirtualRelay`) before the `guideStore.loadAll` call at `AppState.swift:1162`, mirroring what `fetchAllGuides()` already does. Small, safe, stops the errant request outright — but doesn't give a discovering instance anything to show for the relay beyond what `/lineup.json`'s existing `HdhrVCRplusShowTitle` extra already carries.
