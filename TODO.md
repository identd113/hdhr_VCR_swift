...
**This already had a concrete, confirmed bug backing it, not just a nice-to-have — now fixed.** `docs/VirtualTunerService.md`'s own "Known limitation" section documents `guideByDevice[relayId]` as intentionally never populated (`AppState.recordableDevices` is the accessor essentially every guide-fetch call site filters through specifically to exclude relay devices) — but `probeForNewDevices()` (`AppState.swift`, ~line 1157) missed it: `newDevices = (found ?? []).filter { !existingIDs.contains(...) }` had no `isVirtualRelay` filter, so a **newly-discovered** relay device flowed straight into `guideStore.loadAll(devices: newDevices, ...)` — the one guide-fetch call site of the several in `AppState.swift` that wasn't scoped to `recordableDevices` (contrast `fetchAllGuides()`/`performFetchAllGuides()`, both correctly `recordableDevices`-scoped). Live-caught 2026-09-06 in exactly this shape: `[FEED04BE] load() called — DeviceAuth:nil LocalIP:'10.0.2.100' hours:24` immediately followed by a `NETWORK ERROR` (`A TLS error caused the secure connection to fail`, failing URL `https://10.0.2.100/guide.json?...`) — a real, if harmless-in-practice, wasted request against a same-LAN address that has no such endpoint (this app's own `WebServer` is HTTP-only and doesn't implement `/guide.json` at all), happening the instant another instance's FEED is first discovered.

**Fixed 2026-09-07** (flagged again during pre-release review, since this same release's near-real-time FEED discovery push — `onFeedAnnounce` → `probeForNewDevices()` firing immediately on any unsolicited FEED broadcast, not just the ~10s idle-loop poll — made this fire far more often): `newDevices` is now filtered to `guideFetchDevices = newDevices.filter { !$0.isVirtualRelay }` before the `guideStore.loadAll` call, mirroring what `fetchAllGuides()` already does. `devices.append(contentsOf: newDevices)` and `fetchAllLineups(for: newDevices)` stay unfiltered — a relay device still needs to appear in `devices` and still needs its own `/lineup.json` fetched, both real/legitimate; only the SiliconDust cloud-guide fetch was ever the problem. Stops the errant request outright — doesn't give a discovering instance anything to show for the relay beyond what `/lineup.json`'s existing `HdhrVCRplusShowTitle` extra already carries, which is what the fuller feature below is for.

**The fuller feature** this TODO is really about: have `VirtualTunerService`/`WebServer` serve a deliberately minimal guide (one entry: the currently-relayed show, on its one channel — everything already known and already carried in `/lineup.json`'s `HdhrVCRplusShowTitle`/`HdhrVCRplusShowTitle`-adjacent fields, `docs/VirtualTunerService.md`'s wire-protocol notes) from a new endpoint (or reuse `/lineup.json`'s existing per-channel data directly, no new endpoint at all, if that's already sufficient for whatever UI would consume it), and have the discovering side populate a minimal `guideByDevice[relayId]` entry *from that*, never from `GuideStore.load()`'s normal cloud-guide-API/device-`/guide.json` machinery. Not yet scoped: what UI (if any) actually wants to read `guideByDevice[relayId]` once it exists — today nothing does, by the same "Known limitation" note above, since the relay is never listed as a pickable tuner anywhere a guide would render. Needs a concrete consumer in mind before designing the data shape, not just filling in the dictionary for its own sake.
