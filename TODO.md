...
### Simplification ideas for the FEED client-side local relay, if `FeedRelayPacer` confirms pacing is the real fix

Raised 2026-09-12, explicitly **not to be acted on now** — written down for later, once the pacing fix (`ISSUES.md`'s "VLC-side FEED playback stalls" entry) is actually live-verified. Today's implementation (puller curl → local temp file → `streamGrowingFile`/`pumpGrowingFile` re-serve, `WebServer.FeedRelayPacer` smoothing delivery) was built to test a specific hypothesis under real time pressure across several iterations in one session — worth a lightening pass once the dust settles, not because anything in it is wrong, but because a few pieces may be more machinery than the now-understood problem actually needs.

1. **Consider dropping the disk round-trip entirely.** The whole reason FEED needed a *local* relay at all was to give something a chance to smooth delivery before VLC ever sees it — VLC's own libvlc socket read can't be paced directly. But that doesn't require a temp file: this same file (`WebServer.swift`) already has a proven pattern for exactly this shape — `pumpTranscodeProxy`/`beginTranscodeRelay` streams an HTTP source (libvlc's own sout httpd) straight through to an external viewer's `NWConnection` via `URLSession.bytes(for:)`-style streaming, no disk involved. The FEED puller could do the same thing: an in-process streaming fetch of the remote Mac's `/auto/v<channel>` URL, paced through an in-memory buffer, forwarded directly to VLC's connection. That would remove, wholesale: `RecordingManager.startFeedPull`/`stopFeedPull`/`isFeedPullRunning`/`stopAllFeedPulls`/`feedPullPids` (an entire curl-process-lifecycle subsystem), the temp-file naming scheme and `AppState.sweepOrphanedFeedRelayTempFiles()`'s startup sweep, and the disk I/O itself (two extra read+write round trips per byte, on top of two HTTP hops). `FeedRelayPacer`'s pacing math would need to move from "throttle a disk-file read" to "throttle delivery from an in-memory queue fed by URLSession's delegate callbacks" — a different shape, but not more code, likely less. Not attempted today because reusing `streamGrowingFile` (already proven, already handling every edge case — cancellation, deadlines, disconnects) was the faster way to test the pacing hypothesis under time pressure; worth revisiting once the hypothesis itself is confirmed.
2. **`catchUpToLive` reconnecting at `start=0` every time, replaying the whole session from byte one.** Confirmed live 2026-09-12: a reconnect after several minutes has to redrain tens of MB of backlog before reaching the live edge again (harmless today only because backlog drain isn't rate-limited and plays back cleanly regardless of speed — see the static-file test in `ISSUES.md`). If the local relay survives in something like its current disk-backed shape, this should instead reconnect near the live edge (mirroring `AppState.recordingLiveEdgeBackoffSeconds`'s existing precedent for Watch Now) rather than always replaying from the beginning of the session's own temp file.
3. **Re-evaluate `WebServer.feedLiveEdgeCushionBytes`** (currently `0`, kept only "in case a future investigation..." per its own doc comment) now that the raw cross-machine passthrough route (`handleVirtualTunerStream`) it was built for has a different client than it did when it was designed — a puller curl now, not VLC directly. Whether that cushion would actually help *there* is genuinely unknown, not established either way; worth a fresh look rather than assuming the old "made things worse" finding (measured against VLC as the direct client) still applies unchanged.
4. **If idea 1 is done, re-check whether `streamGrowingFile`'s `stillActiveCheck` generalization is still needed at all** — it exists specifically so the FEED-local-relay path (no `show_id`) can plug in `RecordingManager.isFeedPullRunning`. An in-memory relay might have a simpler native "is this still going" signal (e.g., the URLSession task's own state) that doesn't need this parameter threaded through three functions at all. Not clearly a win either way — the closure is small and the two existing callers already pass one trivially — just worth checking rather than assuming it has to stay.
