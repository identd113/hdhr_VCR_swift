...
- Server-side `sent N MB so far` milestones arriving much slower than expected for the channel's
  real bitrate — the original symptom of the now-fully-resolved VLC stall investigation. Full
  trail, root cause, and fix: `issues_resolved.md`'s "VLC-side FEED playback stalls" entry.

**Investigation narrative moved out, 2026-09-13** — this file used to continue with the full
blow-by-blow of that investigation (every theory tried, live-tested, and ruled out on the way to
the actual fix). That trail is now fully consolidated into `issues_resolved.md`'s own entry, in
much shorter form, so it isn't duplicated in two places at very different levels of detail. This
file stays as the reusable "how to actually run a cross-machine FEED test" runbook above — still
accurate and still the fastest way to set one up (e.g. for the still-open audio-track-switching
question in `ISSUES.md`), independent of which specific bug prompted the last time it was used.
