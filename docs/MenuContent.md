...
- **Relay row, added 2026-09-04** — `"FEED: N watching"`, shown only while `state.activeVirtualTunerDeviceID != nil` (this instance's own virtual tuner is currently advertised — a show is recording and the relay is enabled). Behind `AppConfig.FEED_feature_enabled` (default on as of v2.5.0, config-file only) — `activeVirtualTunerDeviceID` can only ever be set by `updateVirtualTunerPresence()`, which is itself gated behind that same flag, so this row stays unreachable if that flag is ever hand-flipped off; same hidden-flag note as the "Recording on Another Mac" section below. `N` sums `state.relayRawViewerCount` (raw-passthrough relay connections, tracked in `AppState` — see its own doc comment) with `VLCBridge.shared.transcodeViewerCount(showId:)` summed across every currently-recording show (already correctly ref-counted per show; not folded into the same running counter, since that accounting already exists and doesn't need a second copy). Shown even at 0 watching, as confirmation the relay itself is up, not only once someone connects. `secondaryLabelColor`, non-interactive, same visual weight as the status message row above it.
...
**Recording on Another Mac (Beta)** section — visible by default as of v2.5.0, behind `AppConfig.FEED_feature_enabled` (config-file only — see `docs/VirtualTunerService.md`'s top note): `AppState.remoteRelayEntries` short-circuits to `[]` whenever that flag is off, so this section (and the status light's blue `feedAvailable` state above) never appears even if a remote relay is genuinely discovered. The rest of this entry describes it as it renders with the flag on (the shipped default — only visible when another discovered hdhrVCRplus instance is relaying an in-progress recording — see `docs/VirtualTunerService.md`; this instance's own relay is never shown here, only a different Mac's):
- Section header: `"Recording on Another Mac (Beta)"` — the "(Beta)" suffix added 2026-09-07; the playback-stall issue is fixed and CC track switching confirmed working (2026-09-13), but audio track switching on a FEED session is still an open, unconfirmed question — see `ISSUES.md`
- One row per remote relay channel: `"Recording on <Show Title> — <source hostname>"` (the hostname suffix added 2026-09-07, from `LineupEntry.virtualRelaySourceHostname`/`HdhrVCRplusSourceHostname` — omitted, not shown as "— unknown", on the rare chance an older source build never set it) with a `play.tv.fill` icon in blue (same visual treatment as the Watching section above) — a `Menu`, not a flat button, opening onto:
  - A **Watch** item — disabled + dimmed, labeled `"Watch (Requires VLC)"`, when `VLCBridge.shared.isAvailable` is false; otherwise calls `AppState.watchRemoteRelay(url:title:device:)` with the entry's plain `URL`, opening a native player window directly against the relay's raw stream. **VoiceOver accessibility label, added 2026-09-07**: `gatedLabel(watchInAppLabel(title), met:vlcReady, requirement:"VLC")` (`"Watch <title>"`, or `"Watch <title> (Requires VLC)"`) — the bare visible `"Watch"` text alone doesn't say which show, since VoiceOver navigating directly to this item (not by first opening the parent `"Recording on <title>"` submenu) wouldn't otherwise hear that context; mirrors `WatchNowView`'s own already-established labels for the same conceptual action (`docs/WatchNowView.md`).
  - **Label changes to `"Watch (H.264)"` when the source is already a modern codec, 2026-09-13** — `MPEGVideoStreamType.isAlreadyModernCodec(codec)` (an unset/`"unknown"` codec is treated as *not* confirmed modern); this is exactly the raw stream you'd get anyway (see "Already-modern-codec skip" below), so the plain Watch button already *is* the H.264 stream — labeling it that way directly reads more clearly than a plain `"Watch"` plus a separate `"You'll get: H264"` footnote explaining what clicking it actually gets you (the footnote is gone; see below). VoiceOver label switches to `watchInAppH264Label(title)` the same way. Codec string formatted for display via `VLCPlayerView.displayCodecName(_:)` (`"H264"` → `"H.264"`), shared with that view's own "Native" popover so the two surfaces can't drift on how a codec name is shown.
  - **A second, separate `"Watch (H.264)"` item, added 2026-09-04** — same VLC gate, shown only when the source is *not* already a modern codec (the bullet above covers the already-modern case instead) — a source that's already H.264/HEVC would just get relayed as-is regardless (`docs/VirtualTunerService.md`'s "Already-modern-codec skip"), making a second, functionally identical button pointless. Calls the same `watchRemoteRelay`, with `&transcode=auto` appended to the URL — any non-empty, non-`"none"` string only tells the *remote* relay "transcode this," it never picks the level; that's always the source Mac's own configured "Default transcode level" (`docs/VirtualTunerService.md`'s `effectiveTranscodeProfile`). VoiceOver label: `gatedLabel(watchInAppH264Label(title), met:vlcReady, requirement:"VLC")` (`"Watch <title> in H.264"`), same reasoning as the plain Watch item above.
  - A divider, then `"Source: <codec>"` (always shown, always accurate). The separate `"You'll get: <codec>"` footnote this used to also show when already-modern (both watch options would otherwise have been identical) was removed 2026-09-13 — the Watch button's own label now says the same thing directly, so the extra line was redundant.
  - **`Show.effectiveVideoCodec`, added 2026-09-13**: `codec` above (published by the source Mac as `/lineup.json`'s standard `VideoCodec` field) now accounts for a real hardware transcode profile applied to this specific recording (`show_transcode`), not just the channel's own raw broadcast codec — a channel that broadcasts MPEG2 but was recorded with a "heavy" (or other) EXTEND transcode profile now correctly reports `H264` here, matching what the file on disk actually is. Previously this field reflected only the channel's native encoding, so a hardware-transcoded recording on such a channel showed the wrong codec (and offered a pointless, already-redundant "Watch (H.264)" button) here and in `VLCPlayerView`'s "Native" popover alike.
...
[Watching FEED from <hostname> — secondary color, only when nowWatchingInfo's device isVirtualRelay]
[Status message — secondary color, uses availableDeviceCount]
[FEED: N watching — secondary color, only when state.activeVirtualTunerDeviceID != nil]
...
Divider
[Now Watching — "Ch 5.1  NBC · Show Title", play.tv.fill icon, only when nowWatchingInfo is non-nil]
...
6. Divider
7. **Watch Now!** and **Watch from Beginning** — always shown, but disabled with a `" (Requires VLC)"` label suffix and dimmed (`disabledControlTextColor`) when `VLCBridge.shared.isAvailable` is false (VLC app not installed or dylib failed to load). Both call `state.watchRecordingInApp(show)` / `state.watchRecordingInApp(show, fromBeginning: true)` (not the generic `watchInApp`) instead of opening a second tuner stream, since HDHomeRun allocates one tuner per TCP connection with no way to share a stream between the recording and a watch session (see `docs/HDHRFindings.md`). Both play it via the WebServer's `/api/watch-recording` relay (open-ended HTTP stream — see `docs/WebServer.md`), differing only in starting offset (~30s behind live vs. byte/second 0 — see `docs/AppState.md`'s `watchRecordingInApp`), and fall back to opening a live stream only if the recording file is missing. `WatchNowView`'s own recording rows offer the same Watch Now!/Watch from Beginning choice as two separate stylized buttons, not a pull-down `Menu` (tried first, didn't match the rest of the action row's visual language) — see `docs/WatchNowView.md`'s Action row. **VoiceOver accessibility labels, added 2026-09-07**: both carry `show.show_title`-specific labels reusing `WatchNowView`'s own existing helpers (`docs/WatchNowView.md`) — `gatedLabel(watchLiveLabel(show.show_title), met:vlcReady, requirement:"VLC")`, `gatedLabel(watchFromBeginningLabel(show.show_title), ...)` — previously the bare visible text ("Watch Now!", etc.) was the only thing VoiceOver announced, ambiguous when multiple recordings each offer the same buttons. **"Watch in VLC" (external VLC.app) removed 2026-09-12** — see `CHANGELOG.md`'s "Unreleased" section for why; this app still requires VLC.app to be installed (for the in-app player's own libvlc dependency), just no longer offers a way to launch it as a separate window.
...
4. Returns `(device: HDHRDevice, channel: LineupEntry, entry: GuideEntry?)`

**Fixed 2026-09-12 — previously silently matched nothing during an active FEED session.** `vlcCurrentURL` is driven by a Combine sink in `AppState.init` that maps `VLCBridge.shared.$currentURL` (the URL actually loaded by libvlc) through `.urlBase` and assigns it to `$vlcCurrentURL` — but once the FEED client-side local relay is in play (`docs/VirtualTunerService.md`'s "Client-side local relay"), `VLCBridge.currentURL`/`vlcCurrentURL` hold a *local* `http://127.0.0.1/api/feed-local-relay?...` URL, which never matches any device's own real lineup entry. `nowWatchingInfo` now checks `VLCPlayerWindowManager.currentFeedRemoteURL` (the tracked true remote URL, non-nil only for an active FEED session) first — the same fix `VLCPlayerView`'s own `currentFeedEntry` already applied for the in-player toolbar. `vlcCurrentURL` still updates automatically whenever VLC starts or stops playing, and still clears to `""` on window close via the same Combine chain (`releasePlayer()` → `currentURL` becomes `nil`) — this button disappears without any explicit clearing in `playerWindowDidClose` either way.

**"Watching FEED from `<hostname>`", added 2026-09-12** — a second, non-interactive row in the header area (see "Header rows" above), shown whenever `nowWatchingInfo`'s matched device `isVirtualRelay`. This is the *reverse* direction from the header's own per-device rows (this Mac's real tuners) and the "Relay row" (other Macs watching *this* Mac's relay) — it's what tells this Mac's own user "you are currently the one sharing someone else's tuner." `watchingRemoteFeedHostname` reads the hostname from `LineupEntry.virtualRelaySourceHostname` (the same `/lineup.json` extra the "Recording on Another Mac" section's own hostname suffix uses), falling back to the bare DeviceID if somehow absent. `secondaryLabelColor`, same visual weight as the other header rows.
