...
**Recording on Another Mac (Beta)** section — **hidden as of 2026-09-10** whenever `AppConfig.FEED_feature_enabled` is off (default, config-file only — see `docs/VirtualTunerService.md`'s top note): `AppState.remoteRelayEntries` short-circuits to `[]` while the flag is off, so this section (and the status light's blue `feedAvailable` state above) never appears even if a remote relay is genuinely discovered. The rest of this entry describes it once the flag is on (only visible when another discovered hdhrVCRplus instance is relaying an in-progress recording — see `docs/VirtualTunerService.md`; this instance's own relay is never shown here, only a different Mac's):
- Section header: `"Recording on Another Mac (Beta)"` — the "(Beta)" suffix added 2026-09-07, since the underlying FEED relay/playback still has known open issues (occasional playback stalls, audio/CC track switching not taking effect — see `ISSUES.md`)
