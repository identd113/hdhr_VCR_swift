...
**`managedShow` computed property** — looks up the scheduled show for this guide entry, used by the action row's Edit/Record button branch and the recording-aware Watch menu (see below). Checks `state.managedShowBySeriesID` first (when `entry.SeriesID` is non-empty), then falls back to `state.managedShowByTitle[entry.Title]` — a `[String: [Show]]` dict (multiple shows can share a title, e.g. "News" on different channels). From the array, picks the first entry that is either a series show (title match is sufficient regardless of channel) or matches the specific `device.DeviceID` + `channel.GuideNumber`. This prevents a show scheduled on ch 5.1 from shadowing an unscheduled "News" on ch 9.1. **Not** used to compute `ringState` (see below) — that's resolved independently via `ManagedGuideMatcher`, a more accurate, device-scoped lookup; deliberate non-sharing, not an oversight.
...
- **Action row** (`.controlSize(.small)`); each button has an `.accessibilityLabel` that includes the show title so VoiceOver can distinguish rows:
  - **Watch** (`.borderedProminent`, `watchNowBlue`) — shown only when `VLCBridge.shared.isAvailable`. For a show this app is **not** currently recording, a plain button calling `state.watchInApp(url:title:deviceId:)`, label `"Watch [title]"`. For a show it **is** currently recording (`managed?.show_recording == true`), a pull-down `Menu` instead — opening the plain live-tuner stream here would open a second, redundant tuner connection for a channel already being recorded. The menu offers **"Watch from Beginning"** (`state.watchRecordingInApp(show, fromBeginning: true)` — relays the recording file from disk starting at byte/second 0, label `"Watch [title] from the beginning"`) and **"Watch Now"** (`state.watchRecordingInApp(show)` — relays from disk starting ~30s behind live, matching the menu bar's own "Watch Now!" default, label `"Watch [title] live"`). Both route through the `/api/watch-recording` relay (see `docs/WebServer.md`'s "Recording playback relay"), which already handles a still-growing file correctly (polls rather than closing on catch-up) — this only changes the starting offset, not the streaming mechanism.
