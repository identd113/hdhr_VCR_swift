...
| `addShow(_ show: Show)` | Appends to `shows[]`, saves config, and broadcasts `webServer.broadcastGuideChangeEvent(type: "show_added", ...)` itself — every caller gets the web-UI push unconditionally (the native Add Show wizard's `save()` and `addShowFromGuide` both just call this and rely on it to broadcast; neither broadcasts separately anymore). If the show is airing right now (`show_next <= now+10 && show_end > now`), fires `startRecording` immediately via a `Task` that re-derives the index by `show_id` rather than capturing one, so an interleaved mutation before the Task runs can't act on a stale index. |
