...
The same class of fix applies to two other functions that hold an index across an `await`:
...
5. **Recording ends** — all three terminal outcomes (natural completion with data, the empty-output-file failure, and the fail-threshold "⏸ Recording Paused" event) go through `fireDiscordCard(..., clearIdAfter: true)`. The natural-completion path previously called `discordShow(editMessageId:)` directly, bypassing `discordCardInFlight` entirely — if a "Recording Started" CREATE was still in flight (slow webhook) when the recording finished, that direct call could read `discord_start_msg_id` as still empty and fire a second, ID-less "Recording Complete" POST via `discordShow`'s fire-and-forget path, orphaning it once the Started card's id landed afterward with nothing left to clear it. Routing through `fireDiscordCard` closes that gap the same way it does for the other two terminal events. In all cases `discord_start_msg_id` is cleared to `""` after the terminal event.
