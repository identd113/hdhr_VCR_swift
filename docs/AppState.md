...
| `tunersFull(for: deviceId)` | Returns `true` when every tuner on the device is occupied. Counts **both** active recordings (`recordingShows`) and the in-app VLC stream — via the private `vlcOccupiesTuner(for:)` helper, not a bare `currentDeviceID` check (see below). Use this — never count recordings alone — because watching and recording occupy tuners equally, *except* watching a recording via the relay. Used by `startRecording`, `WatchNowView`, and `WebServer.handleRecord`. |
