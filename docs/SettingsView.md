...
### Category: General
Two `Section`s:
- **System**: `Toggle("Launch at Login")`, `Toggle("Blink menu bar icon")`, `Picker("Appearance")` — bound to `draft.Appearance_mode` (`"auto"` | `"light"` | `"dark"`, default `"auto"`). Applied via `.preferredColorScheme(_:)` to every one of this app's own `Window` scenes (`hdhr_VCRApp.swift`'s `resolvedColorScheme`, `nil` for `"auto"` so a window just follows the system the normal SwiftUI way) — takes effect live the moment it's saved, no relaunch needed, since every window scene reads `appState.config` directly. Also applied to the web guide when it's loaded inside this app's own embedded `WKWebView` (`AddShowView`'s guide step — see `docs/AddShowView.md`'s "Appearance sync"), two-way: that embedded instance's own theme-switcher buttons update this same setting too. Does **not** reach a browser connecting over Web LAN — that's a structurally separate client with no channel back to this setting at all, not something this toggle has to specifically exclude (`docs/WebServer.md`'s dark-mode section).
- **Guide**: `Toggle("Use XMLTV guide format")` — bound to `draft.Guide_use_xml` (default `false`). Moved here from the Guide tab's former Format section (2026-08-10) so the JSON/XMLTV switch is visible without digging into a dedicated tab. While the toggle is on, an inline amber warning (the shared `CaveatBanner` component — soft orange-tint fill, orange text, the same one `ShowFormSection`'s transcode-mismatch banner uses, `accessibilityIdentifier("settings-guide-xmltv-warning")`) surfaces XMLTV's real caveats: Guide Hours is ignored (the server always sends ~2 days), and any device without DeviceAuth silently falls back to JSON regardless of this setting. Toggling and saving triggers an immediate guide refresh via the XMLTV endpoint.
...
- `"Update Guides Now"` `.borderedProminent` button — swaps for a small `ProgressView` spinner for the duration of the refresh (`@State guideRefreshInProgress`), same convention as the Maintenance tab's `maintenanceRow` spinners and the Discord webhook "Test" button's `.testing` state
...
- **Write metadata sidecar** — `Toggle` (off by default). When enabled, `startRecording` (`AppState.writeMetadataSidecar(path:show:entry:)`) writes a Kodi-style `<episodedetails>` `.nfo` XML file next to the recording (same folder, same basename, `.nfo` in place of `.ts`) right after a successful launch — same guide entry already resolved for the series-subfolder episode tag above, `guideEntryForShow(show)`, shared rather than looked up twice. Fields: `<title>` (episode title, falling back to the show title), `<showtitle>`, `<season>`/`<episode>` (parsed from `EpisodeNumber`; `-1` when it isn't a full `SxxExx` tag — Kodi's "unknown" convention), `<aired>` (UTC `yyyy-MM-dd` from `OriginalAirdate`, omitted if absent), `<plot>` (Synopsis, omitted if empty), `<genre>` (`firstGenre`, omitted if empty), `<runtime>` (minutes, omitted with no guide entry at all). Written unconditionally when there's no matching guide entry — title-only `.nfo`, not skipped — so every recording made with the toggle on gets one. A write failure is logged as a warning and never fails the recording itself. Stored in `draft.Write_metadata_sidecar`.
...
**Fetch section:**
- **Show next N hours** — `Stepper` (1–28, capped below the old 1–48 range). Controls only how far ahead each fetch requests (`GuideStore.loadAll(hours:)`) — **not** how often it fetches. Capped at 28 because `GuideStore.load()` makes a single API call and the cloud `guide.php` endpoint silently truncates any single request past ~29h (`docs/HDHRFindings.md`) — a higher setting would look accepted but never actually fetch further out. `AppConfig.init(from:)` also clamps a decoded value from an older config that saved something above 28.
- **Auto-refresh** — `Picker` offering 1/2, 1/4, or 1/8 (`Guide_refresh_interval_divisor`, default 8), each option label showing its live-computed *nominal* duration with a `~` prefix (e.g. "1/8 (~3 hr)") via `formattedGuideRefreshInterval(divisor:)`, which calls only `AppState.guideRefreshIntervalSeconds(guideHours:divisor:)` — the same nominal-interval calculation the idle loop uses, so the preview's baseline number can never drift from the app's own math — and updates live as the "Show next N hours" stepper above changes. The `~` is deliberate: the *real* scheduling function, `AppState.effectiveGuideRefreshIntervalSeconds` (which the preview does not call — see `docs/AppState.md`'s idle-loop entry for its full formula), randomizes the actual fire time within the final hour of that nominal interval and re-floors the result at 30 min, so "~3 hr" means "sometime between 2h and 3h, never less than 30 min," not exactly 3h. Deliberately expressed as a fraction of `GuideHours` rather than an independent duration (changed 2026-09-21, was a 15–240 min `Stepper` added the day before): a wider fetch-ahead window can afford to refresh less often before running dry; a narrower one needs to refresh more often. `AppState.idleLoop`'s elapsed-time gate against `lastGuideRefreshAt`/`guideRefreshTargetSeconds` is what this drives. "Update Guides Now" always bypasses this for an immediate manual refresh.
...
- **Update Guides Now** — `Button` (hidden while a refresh is in flight, replaced by a `ProgressView` spinner). Awaits `state.refreshAll()` (now `async`, previously fire-and-forget) immediately, invalidating and reloading guide data for all devices. Useful any time fresh data is needed without restarting the app.
...
Three independent sharing *methods* — **Web LAN** (the LAN web server itself), Terminal Guide, Recording FEED — each with its own toggle that reveals its own options directly beneath it when on. Recording FEED's whole section is gated behind `FEED_feature_enabled` (default on as of v2.5.0 — see its own entry below) — Web LAN and Terminal Guide stay always-visible. The Web LAN toggle is the one durable, user-controlled capability; Terminal Guide's toggle is `.disabled` whenever it's off (Terminal Guide connects to that exact same internal web server, no separate listener of its own); Recording FEED's toggle is **not** gated on it at all — see its own entry below for why. All three of the actual per-method toggles (`Virtual_tuner_relay_enabled` included) default off — a user still has to opt this specific Mac in — and are each explained, with their own purpose-built animated diagram, on first launch by `FirstRunWizardView`'s Web LAN, Terminal Guide, and Recording FEED steps (three separate steps, not combined — see `docs/FirstRunWizardView.md` for why) — see `docs/FirstRunWizardView.md`.
...
- **Terminal Guide section** — always visible; its **Enable Terminal Guide** `Toggle` (bound to
  ...
  **Why the dependency is real, not just a UI choice**: `hdhr_guide` (`Sources/hdhr_guide/`)
  connects to the exact same internal `NWListener` Web LAN exposes — there's no second
  ...
  an unrelated reason (an active Recording FEED, an open WKWebView guide window), but a Settings
  toggle needs a *durable* guarantee, not a coincidence that could disappear the moment a recording
  ends — so this gates on the persistent Web LAN toggle specifically, not on `webServerRunning`.
  ...
  to any LAN browser once Web LAN is on regardless of this flag (see `docs/TUIGuide.md`). When both
  the Web LAN and Terminal Guide toggles are on: if the server is actually live
...
- **Changelog** — rendered by `MarkdownView: NSViewRepresentable` (private struct at file scope). Parses via `AttributedString(markdown:options: .init(interpretedSyntax: .full))`, then `MarkdownView.render(_:)` walks each run's `PresentationIntent` (`intent.components`, which runs `[paragraph, listItem(ordinal), unorderedList|orderedList, ...ancestors]` — verified directly against Foundation, not documented by Apple) to reproduce block structure the naive `NSAttributedString(AttributedString)` bridge drops on the floor: `## ` headers get bold/enlarged text on their own line, `- ` list items get a hanging-indent "•\t" bullet (`1. ` ordered items get a real "N.\t" number instead — the immediately-enclosing list's orderedness is read off the list-kind component right after `.listItem` in that chain; nested lists indent further per enclosing list level), consecutive list items get tight single-line spacing while every other block transition gets a blank line, and any run left with no `.foregroundColor` (i.e. everything except markdown links) is filled in with `NSColor.labelColor` so it tracks light/dark mode. Inline styling (bold/italic/inline code/links) still comes through automatically via the per-run `NSAttributedString(AttributedString)` bridge. Displayed in a non-scrollable `NSTextView` (selectable, no background). Height is self-measured via `layoutManager.usedRect(for: textContainer)` after each layout pass and injected as a `.frame(height:)` — each `MarkdownView` instance self-sizes to its own content (see the next bullet for why there are now two instances, each with its own height floor of 40, not one at 100). Only sections at or below the running build version are shown.
  - **Current-version highlight + 6-section cap** — `SettingsView.parseChangelog(_:maxSections:)` (widened from `private` to `internal` for direct test coverage, same precedent as `HDHRManager`'s methods) now returns an array of up to `maxSections` (default 6) independently-renderable "## "-prefixed section strings, newest first, instead of one joined blob — `CHANGELOG.md` itself and the version-filtering logic (nothing newer than `appVersion` is ever shown) are untouched, this is purely a display cap. `aboutView` renders `sections.first` (the current build's own entry — always unconditionally kept regardless of date, a pre-existing behavior this cap doesn't change) in its own `MarkdownView` instance inside a `Label("Current Version", systemImage: "checkmark.seal.fill")`-tagged box with an accent-tinted background/border (`Color.accentColor.opacity(0.12)` fill, `0.4` stroke); `sections.dropFirst()` (up to 5 older entries) render below in a second, unstyled `MarkdownView`, joined back with `"\n\n"`. Each of the two `MarkdownView` instances self-measures its own height independently (`currentChangelogHeight`/`changelogHeight`), since they're now two separate `NSTextView`s rather than one.
- **GitHub link** — `Link("View on GitHub", destination: URL(...))`
- **Credits** — added 2026-09-04: a `Divider()`, a `"Credits"` headline, then "Playback powered by [VLC media player](videolan.org/vlc/)" plus a secondary-colored "© VideoLAN, licensed under the GPL/LGPL." line. Attribution for a runtime dependency, not a bundled one — `VLCBridge.swift` `dlopen`s `libvlc.dylib` from the user's own separately-installed `VLC.app`, never bundled/redistributed inside `hdhrVCRplus.app` (see `docs/MAS_COMPLIANCE.md`'s "VLC in-app player (dlopen)" entry for why bundling would be a different, much harder licensing story).
