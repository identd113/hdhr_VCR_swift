...
- **Auto-refresh every N min** — `Stepper` (15–240, step 15; `Guide_refresh_interval_minutes`, default 60). How often the background auto-refresh (`AppState.idleLoop`'s elapsed-time gate against `lastGuideRefreshAt`, independent of the window-size setting above) re-fetches lineup+guide, to keep rolling the lookahead window forward as time passes (otherwise the fetched range would shrink toward "now" between refreshes) and to pick up real-world schedule corrections (preemptions, delayed starts). Added 2026-09-20 (previously hardcoded to a fixed hourly clock-boundary check with no user control). "Update Guides Now" always bypasses this for an immediate manual refresh.
