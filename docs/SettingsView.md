...
Three independent sharing *methods* — LAN, Terminal Guide, Recording FEED — each with its own always-visible toggle that reveals its own options directly beneath it when on. LAN is the one durable, user-controlled capability toggle; Terminal Guide's toggle is `.disabled` whenever LAN is off (it connects to that exact same internal web server, no separate listener of its own); Recording FEED's toggle is **not** gated on LAN at all — see its own entry below for why.
