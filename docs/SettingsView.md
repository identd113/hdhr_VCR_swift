...
- **GitHub / Discord links** — `Link("View on GitHub", destination: URL(...))` and `Link("Join Discord", destination: URL(...))` (`https://discord.gg/UMp57Nmgbd`), side by side in one `HStack`, both `.buttonStyle(.bordered)`
