...
- **Auto-refresh** — `Picker` offering 1/2, 1/4, or 1/8 (`Guide_refresh_interval_divisor`, default 8), each option label showing its live-computed *nominal* duration with a `~` prefix (e.g. "1/8 (~3 hr)") via `formattedGuideRefreshInterval(divisor:)`, which calls the exact same `AppState.guideRefreshIntervalSeconds(guideHours:divisor:)` the idle loop's nominal-interval calculation uses — the preview can never drift from the app's own math, and updates live as the "Show next N hours" stepper above changes. The `~` is deliberate: the actual fire time is randomized within the final hour of that nominal interval (`AppState.jitteredGuideRefreshIntervalSeconds`, re-rolled every cycle) rather than always landing at the same offset, so "~3 hr" means "sometime between 2h and 3h," not exactly 3h. Deliberately expressed as a fraction of `GuideHours` rather than an independent duration (changed 2026-09-21, was a 15–240 min `Stepper` added the day before): a wider fetch-ahead window can afford to refresh less often before running dry; a narrower one needs to refresh more often. `AppState.idleLoop`'s elapsed-time gate against `lastGuideRefreshAt`/`guideRefreshTargetSeconds` is what this drives. "Update Guides Now" always bypasses this for an immediate manual refresh.
