...
- **Skip already-recorded episodes** — `Toggle` (off by default), `.disabled(!draft.Series_subfolder_enabled)` so it greys out unless Series subfolders is on. When enabled, `startRecording` checks whether a file with the upcoming episode's season+episode (`SxxExx`) already exists in the show's folder tree (via `AppState.recordedEpisodeTags(forTitle:baseDir:)`); if so it logs `SKIP`, optionally posts a Discord card (`Discord_on_duplicate`), and advances to the next airing **without** recording or incrementing `show_fail_count`. Only fires when the upcoming guide entry has a full `S\d+E\d+` tag (season-only/blank tags record as normal). Also drives the web guide's green "already recorded" corner flag (`.g-flag-skip`, `docs/WebServer.md`). Stored in `draft.Skip_recorded_episodes`.
