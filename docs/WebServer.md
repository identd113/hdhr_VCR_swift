...
- **`onStreamEnded: (() -> Void)?`, added 2026-09-04** — an optional closure threaded through `streamGrowingFile`/`pumpGrowingFile`/`handleGrowingFileChunk`'s full parameter chain, fired exactly once at whichever of that chain's several exit points the stream actually ends at (a real send error or timeout, the connection dying, the requested duration elapsing, or the source recording finishing and draining). `handleVirtualTunerStream` is the only caller that passes one — right before its `streamGrowingFile(...)` call it bumps `AppState.relayRawViewerConnected()`, and passes `{ Task { @MainActor in appState?.relayRawViewerDisconnected() } }` as `onStreamEnded` (see `docs/VirtualTunerService.md`'s `relayRawViewerCount` entry). `handleWatchRecording` (local Watch Now) passes `nil` and is never counted — this is specifically an outbound-to-another-machine viewer count.
