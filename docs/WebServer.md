...
| *(global `keydown` listener)* — type-to-search | Skips when `_searchShow` is set, a guide modal is open, a modifier key is held, the key isn't a single printable character, or focus is already on `#search-in` or another genuinely editable field. Otherwise: `preventDefault()`s (also blocks Firefox's own quick-find), seeds `#search-in`'s value with the pressed key, focuses it, places the caret after it, and calls `onSearchInput()` |
