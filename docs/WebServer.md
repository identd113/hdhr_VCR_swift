...
Serves an interactive guide page and JSON API over HTTP. The page is consumed by two clients: **external browsers** on the local network, and the **in-app WKWebView** in `AddShowView` step 2. Both connect to the same SSE stream and see the same HTML. Enabled via **Settings → Sharing → Enable Web LAN** (the Settings section's user-facing label — the underlying `Web_server_enabled` config key and `WebServer.swift` itself are unchanged). Default port: **1980**.
...
func refreshPageAndBroadcastGuideChange(type:state:)  // @MainActor — thin wrapper around broadcastGuideChangeEvent (which now does its own grid build + cachedHTML rebuild); kept as a distinctly-named entry point for refreshGuides()'s periodic "guide_refreshed" call site (cadence is AppState.effectiveGuideRefreshIntervalSeconds-derived, not hourly — see "Guide-change events" below)
```
...
- `streamGrowingFile(path:showId:startOffset:conn:)` — sends `200 OK` with `Content-Type: video/mp2t`, **`Connection: close`** (changed from `keep-alive` 2026-09-07 — this response never carries `Content-Length`/`Transfer-Encoding: chunked`, so per RFC 7230 §3.3.3 "read until the connection closes" is the only valid framing; declaring `keep-alive` on top of an unknown-length body was a real spec violation that likely made VLC's HTTP access module read defensively rather than pulling data as fast as it was available — found investigating why a real HDHomeRun tuner's own direct stream, also close-terminated but correctly labeled, played smoothly while this relay didn't), no `Content-Length`; if `startOffset > 0`, seeks the `FileHandle` there first (clamped to the file's current size so a stale offset can't seek past EOF, then rounded down to the nearest complete 188-byte TS packet via `alignedToTSPacketBoundary(_:)` — added 2026-09-06 as a regression fix: `startOffset` is usually the recording file's momentary byte size, which has no relation to TS packet framing since curl writes to disk in whatever chunk sizes its own TCP reads land on, so an unaligned seek used to hand a fresh FEED viewer a torn leading packet, whose demuxer then had to scan forward for the next sync byte — the "plays a beat, stalls, fragments of audio" symptom — before it could decode anything at all); also decides the *initial* chunk size for the pump loop below (`hasBacklog`: does the file's current size at connect time exceed `startOffset` by more than one small chunk?) — this "current size at connect time" is normally a fresh stat taken right here, but an optional `knownFileSizeAtOffsetComputation` parameter (added 2026-09-11) lets a caller that already stat'd the file to derive `startOffset` itself (`handleVirtualTunerStream`'s FEED join-at-live-edge path) reuse that same reading instead of stat'ing again — a second independent stat left a real window (this function's own `FileHandle` open plus the caller's preceding `glog` calls) for the file to keep growing between the two, letting `hasBacklog` flip true even when the caller computed `startOffset` to land exactly on the live edge "by construction," reintroducing the large-chunk burst-delivery pattern the adaptive chunk sizing below exists to avoid; `handleWatchRecording` doesn't have a stat of its own to reuse and still re-stats here as before — then hands off to the pump loop
- `pumpGrowingFile(...)` — checks `conn.state` first on every recursive call (`.cancelled`/`.failed` closes the file handle and stops recursing immediately) — otherwise a connection cancelled while the loop sits in its live-edge wait-for-more-data poll (the common state once caught up) wouldn't be noticed until a `conn.send()` was actually attempted, which only happens once new bytes arrive. Reads and sends chunks via `sendWithTimeout`, at a **per-connection, adaptive chunk size** (reworked 2026-09-07, see below); on an empty read, polls every **20ms** while `show.show_recording == true` (an `@MainActor` "is it still recording" check only runs every 25th poll, ~500ms, to avoid 50 MainActor hops/sec — the interim polls just retry the read directly), then drains and closes once the show stops recording; on a send failure (real error or timeout — see below), closes the file handle, cancels the connection, and stops recursing.
  - **Chunk size — `watchRecordingChunkSize` (8 TS packets, 1504 bytes) at the live edge, `watchRecordingBacklogChunkSize` (200 packets, ~37.6KB) while draining a real backlog, reworked 2026-09-07.** A direct byte-level comparison (a raw-socket sampler logging `recv()` sizes/gaps) against the real HDHomeRun tuner's own port-5004 stream found the real device delivers ~1.5KB chunks with inter-arrival gaps almost always under 20ms; this relay, before this fix, delivered ~37.6KB bursts followed by a hardcoded 500ms silent poll on ~15% of reads — a burst/silence pattern the real device can never produce (a physical broadcast can't accumulate a backlog to burst-release), and a plausible direct cause of the PCR-vs-wall-clock drift VLC's own `--file-logging` debug output named as the actual FEED-stall trigger (see `ISSUES.md`'s FEED entry / `issues_resolved.md`'s matching write-ups for the full trail). `handleVirtualTunerStream`'s FEED path always joins at the live edge (zero backlog by construction), so it always starts and stays on the small size. `handleWatchRecording`'s native Watch Now relay can have a real multi-MB backlog after a scrub-bar seek, so it starts on the large size and downgrades one-way — the moment a non-empty read comes back *shorter* than requested (backlog exhausted) or a genuine empty read is hit (a safety net for a backlog that happened to be an exact multiple of the large size) — to the small size, permanently, for the rest of that connection.
  - The empty-read branch is the one part of the pump loop that needs a `Task { @MainActor in ... }` hop (to read `show.show_recording`) before it can schedule the next poll — see `docs/VLCBridge.md`'s "Channel Switching (play)" for why that specific dependency mattered: it was one half of a real mutual deadlock with VLC's own blocking stop call, fixed 2026-08-15 on the VLCBridge side without needing to change anything here
  - **`liveEdgeCushionBytes` parameter (default `0`)**, threaded through `streamGrowingFile`/`pumpGrowingFile`/`handleGrowingFileChunk` — when `> 0`, both the join offset and every subsequent read ceiling stay that many bytes behind the file's true current size instead of tracking it exactly, and the chunk-size choice above is recomputed against that shifted boundary fresh on every iteration (not one-way-only-shrinks, since a persistent cushion can develop a real backlog again mid-session). `handleWatchRecording` (local Watch Now) always passes `0`; `handleVirtualTunerStream`'s FEED relay passes `feedLiveEdgeCushionBytes`, itself currently `0` (disabled 2026-09-09 after live testing found it made a VLC-side stall worse, not better — mechanism kept, not deleted) — see `docs/VirtualTunerService.md`'s live-edge cushion entry for the full history.
- **`sendWithTimeout(_:on:timeout:completion:)`, added 2026-09-04** — wraps every `conn.send(...)` in this pump (both the initial header in `streamGrowingFile` and each chunk in the loop above) with a caller-supplied `sendTimeout` fallback. `Network.framework`'s own send has no built-in timeout: a peer that stops draining its TCP receive window (rather than cleanly closing) leaves `.contentProcessed`'s completion pending forever, with `conn.state` never transitioning to `.cancelled`/`.failed` either — so the existing per-recursion state check above can't catch it. Caught live via the virtual-tuner relay (this same pump, `handleVirtualTunerStream` → `streamGrowingFile`): a remote viewer's stream froze for ~5 minutes with zero log signal on this Mac's side, while a sibling connection to the same peer stayed perfectly healthy the whole time (ruling out a Mac-wide bottleneck — see `docs/VirtualTunerService.md`), until the client itself gave up and reconnected from byte 0. `sendWithTimeout` races the real `.contentProcessed` completion against a `queue.asyncAfter` timer (both always run on `queue`, so no lock is needed to guard against double-firing); whichever fires first wins, reporting `nil` on success or a failure-reason string (the real error's description, or a synthetic `"timed out after Ns..."`) on either a genuine send error or a timeout. Either failure path now explicitly calls `conn.cancel()` — a real send error usually means the OS already knows the connection is dead, but the timeout case does not, so without this a stalled connection would stay open indefinitely from this side even after giving up on it (`cancel()` is idempotent, safe to call either way).
  - **A caller-specific short timeout for the real remote relay was tried twice, then removed entirely, 2026-09-05.** `streamGrowingFile`/`pumpGrowingFile`/`handleGrowingFileChunk` now all use `growingFileNoTimeout` (86400s, effectively unbounded — matching this file's own "a day stands in for infinite" idiom, `pumpTranscodeProxy`'s `URLSessionConfiguration` timeouts) unconditionally — there is no longer a `sendTimeout` parameter to configure per caller. Originally, only `handleVirtualTunerStream` (the actual outbound relay to a real remote viewer) passed a short `growingFileSendTimeout` explicitly, first 15s then 60s: a first version of the whole `sendWithTimeout` mechanism applied 15s uniformly to every `streamGrowingFile` caller, which also caught `handleWatchRecording`'s internal consumers — local Watch Now, and critically `/api/watch-recording` also being what `VLCBridge`'s own headless transcode session reads as its *encode source* (`docs/VirtualTunerService.md`'s "Real transcode (Phase 2)") — a real-time H.264 encode reads its input in bursts and can legitimately go quiet well past 15s (especially right at startup) with nothing actually wrong, so the aggressive timeout was killing the encoder's own source feed out from under it, tearing the whole transcode session down and forcing a full restart, repeatedly (`"watch-recording ... timed out after 15s waiting for client to accept data"` firing twice in six minutes on that connection — the literal mechanism behind a reported "plays a few seconds, then buffers" symptom). The fix at the time carved the relay out as the one caller still worth an aggressive deadline. Raising it to 60s didn't hold up either: live-testing a real raw (untranscoded) FEED viewer end to end showed the same "timed out" line firing routinely (5 times in one short session) against a perfectly healthy connection that had just sent real data moments before — the receiving Mac's own VLCBridge, playing the relay like any other stream, legitimately stops draining the socket for a stretch once its buffer is comfortably ahead, the exact behavior already known from the transcode-source case, just now observed on the far end of a real network hop instead of the near end of a local one. Killing the connection over this doesn't recover anything a real failure wouldn't already surface via `conn.state` — it just forces an unnecessary reconnect at a new live-edge offset, which is what a "the relay doesn't work" report actually looked like: stutters and skips ahead every 15-30s, not a stream that never starts. The conclusion: a growing recording file should be read and polled until the connection is genuinely over, never cut off on a guess from send timing — so the relay now gets the same unbounded treatment every other caller already had.
- **`onStreamEnded: (() -> Void)?`, added 2026-09-04** — an optional closure threaded through `streamGrowingFile`/`pumpGrowingFile`/`handleGrowingFileChunk`'s full parameter chain, fired exactly once at whichever of that chain's several exit points the stream actually ends at (a real send error or timeout, the connection dying, the requested duration elapsing, or the source recording finishing and draining). `handleVirtualTunerStream` is the only caller that passes one — right before its `streamGrowingFile(...)` call it bumps `AppState.relayRawViewerConnected(showId:)`, and passes `{ Task { @MainActor in appState?.relayRawViewerDisconnected(showId: showId) } }` as `onStreamEnded` (see `docs/VirtualTunerService.md`'s `relayRawViewerCounts` entry). `handleWatchRecording` (local Watch Now) passes `nil` and is never counted — this is specifically an outbound-to-another-machine viewer count.
...
**AppState side:** `watchRecordingInApp(_:)` builds `http://127.0.0.1:{Web_server_port}/api/watch-recording?show={show_id}&start={offset}` — `offset` defaults to ~30s behind the live edge (not byte 0), so Watch Now! starts near "now" rather than at the beginning of the recording — and calls `ensureWebServerRunning()` (the same refcounted internal-use path `AddShowView`'s guide step uses, so this works even with the LAN web UI disabled in Settings) guarded by a `recordingRelayActive` flag; `VLCPlayerWindowManager.playerWindowDidClose()` calls `AppState.releaseRecordingRelayIfNeeded()` to balance it. An external-VLC path (`watchRecordingInVLC(_:)`, gated on a `Watch_in_VLC` setting) used to exist alongside this one — removed entirely 2026-09-12, see `CHANGELOG.md`'s "Unreleased" section — this relay now has exactly one consumer, the in-app player.
...
`guide_refreshed` now falls into case 3 instead of the old fetch-based fallback — the grid the idle loop already rebuilt via `prebuildPageHTML` at guide-refresh time is reused by pushing the identical fragments computed by `broadcastGuideChangeEvent`, so no additional rebuild happens per connected tab. It's broadcast on the cadence `AppState.effectiveGuideRefreshIntervalSeconds(guideHours:divisor:unitRandom:)` produces (`config.GuideHours` ÷ `config.Guide_refresh_interval_divisor`, default 24h ÷ 8 = 3h nominal, floored at 30 min via `guideRefreshIntervalSeconds`) — the actual fire time is jittered within the final hour of that nominal interval (`jitteredGuideRefreshIntervalSeconds`, re-rolled each cycle) and that same 30-min floor is re-applied to the jittered result too (raw jitter alone can dip below it for a nominal under 90 min — see `docs/AppState.md`'s idle-loop entry), user-configurable in Settings → Guide — so an idle guide window does a background refresh on roughly that cadence even with no user activity.
...
**HTML cache:** `prebuildPageHTML(state:)` pre-renders the page HTML and stores it in `cachedHTML` — one shared copy for all UAs, since desktop and mobile now render the same guide window (see below). It also gzips that HTML once at the same time and stores the result in `cachedHTMLGzip`; `GET /` returns `.okPrecompressed(...)`, which picks whichever of the two `send()` already has on hand based on the request's `Accept-Encoding` instead of re-running DEFLATE on every request (the page is ~1.5 MB raw — compressing it costs ~30–60 ms, dwarfing everything else in a LAN page load, so paying that cost once per rebuild instead of once per `GET /` was a meaningful win). Both caches falling back to a live synchronous build (via the generic `.ok(...)` case, gzipped on the fly by `send()` same as any other response) is a documented 2–4 second `@MainActor` blocking time — the whole app freezes, not just the requesting client. **Fixed 2026-09-13**: `AppState.startup()` now calls `webServer.prebuildPageHTML(state: self)` immediately after `setupWebServer()`, well before device discovery or the guide fetch even starts — so in practice both caches are populated (with an accurate, if guide-data-empty, page) within the first second of launch, and the synchronous fallback above is no longer reachable in normal operation. It remains in place only as a defensive backstop (e.g. a future call site that somehow reaches `GET /` before `startup()` runs at all). `AppState.guidePageCacheWarm` (set the moment `prebuildPageHTML` completes) exists specifically so a consumer like `AddShowView`'s embedded guide `WKWebView` can wait for "the cache is actually warm," not just "the listener is bound" — see `docs/AddShowView.md`'s "Guide-page cache warmth" section.

`prebuildPageHTML` is called after every guide load (`fetchAllGuides`, `refreshGuides`), and also from `broadcastRecordingEvent` (every recording start/stop) and `broadcastGuideChangeEvent` (every add/delete/pause/resume/edit/favorite-toggle, plus the periodic guide refresh) — so `cachedHTML` stays current with every state change that affects the grid, not just the periodic guide reload. Without this, a fresh page load (a new tab, a hard refresh, or reopening the native Guide window's `WKWebView`, which does a fresh `GET /` each time it's created) could show a show as not-yet-recording for up to a full guide-refresh interval after it actually started (default ~3h, user-configurable — see `docs/AppState.md`'s idle-loop entry) — connected tabs don't hit this because they get a live SSE class-toggle/grid-swap patch instead of re-fetching the page.
...
**Placeholder** (`#sum-ph`, `buildSumPhHTML`): nothing selected — shows the active recording (if any) plus a same-day "Up Next: &lt;title&gt;" line for the next show airing later today (standardized 2026-09-14 to match `MenuContent.swift`'s Up Next section and `buildTunerShowsHTML`'s — a soonest-active show that isn't today is left out rather than shown), or "Select a show from the guide" when there's neither a recording nor anything left today.
...
**Bonus Time preview overlay (`.g-bonus-overlay`), added 2026-09-14** — a managed guide entry whose owning show has `show_bonus_time` set gets a faint sports-colored wash extending past its own listed end, into whatever time it would actually eat into if it ran the full `Sports_padding_minutes` long — a passive, purely visual heads-up that this recording tends to run over. Gated on `state.config.Sports_padding_enabled` and `owner?.show_bonus_time == true` (`owner` = `guideMatcher.owner(for: e)`, the same managed-show lookup `isMgd`/`data-managed` use) — **not** a bare genre guess (`Show.genreImpliesBonusTime`, still used elsewhere as the *default* when a new show is first added, e.g. `AddShowView`'s auto-check) — narrowed 2026-09-14 from an earlier version that showed this for any sports-genre entry regardless of scheduling, including reruns nobody had scheduled or shows with Bonus Time explicitly turned off. This mirrors the actual gate `AppState.skipRecording`'s own end-time extension uses (`config.Sports_padding_enabled && show.show_bonus_time`), so the preview can't show spillover for a show the recording engine itself wouldn't actually extend. A separate sibling `<div>` appended right after the entry's own `.g-prog` block (not merged into it — `.g-prog`'s `overflow:hidden` would clip anything drawn past its own edge), positioned with the same `--gs`/`--gw` percent system, `left` set to the entry's own rendered end and `width` to `min(EndTime + Sports_padding_minutes*60, winEnd)` minus that. Always the sports color (`var(--gg-sports)`, theme-aware — not the static `.gg-sports` class, an existing light-mode quirk this deliberately avoids inheriting) at `opacity:.3`, `z-index:2` (above a plain block/gap/now-bar, below `:hover`/selection so a real interaction still visually wins), `pointer-events:none` + `aria-hidden="true"` (purely decorative, no effect on click/keyboard behavior of the blocks underneath it or the one it belongs to). The HTML itself is shared by `/` and `/vertical` for free (one `buildGuideGridHTML()` grid feeds both), but the vertical time-axis mode's own transposition CSS (`guide-vertical.css`) needed its own `html .g-bonus-overlay{...}` rule added alongside `.g-prog`/`.g-gap`/`.g-now-bar`'s existing ones — without it this element would've stayed horizontally positioned (using the plain `guide.css` rule) instead of transposing to the vertical axis in portrait mode.
...
**Config staleness:** `_defaultTranscode`, `_bonusEnabled`, and the bonus row's minutes label are baked into the served HTML at page-generation time (same as `_bonusMins`) — a config change in Settings takes effect on the next guide load (page refresh or the periodic `guide_refreshed` reload), not immediately.
...
- **Up Next** — that tuner's first `activeShows` entry by `show_next` ascending, but only if it airs later today (standardized 2026-09-14 to match `MenuContent.swift`'s own Up Next section, `docs/MenuContent.md`) — omitted entirely once nothing's left today, rather than falling back to a later day's next show; relative time in accent color.
...
- Every accepted connection's own request/response I/O and SSE fan-out sends run on a private `DispatchQueue` (`queue` — `hdhrVCRplus.webserver`, `.utility`).
- **`acceptQueue`, added 2026-09-11** — `NWListener` itself (and its `stateUpdateHandler`/`stop()` cleanup) runs on a *separate* queue (`hdhrVCRplus.webserver.accept`, `.userInitiated`) from `queue` above, not the same one. Before this, `l.start(queue: queue)` meant a new connection's `newConnectionHandler` callback sat in the exact same serial FIFO as every other connection's request/response I/O and SSE fan-out sends — a large guide-change broadcast to several connected SSE clients (still real work even gzip'd, see `broadcastGuideChangeEvent` below) could delay accepting a brand-new client behind it (`TODO.md`'s "`broadcastGuideChangeEvent`'s SSE payload" entry, option 2). `acceptQueue` decouples "notice and register a new connection" (`handleConnection`: subnet check, append to `liveConns`, hand off to `queue` for its own I/O) from that fan-out — accept latency no longer scales with how many SSE clients are currently being pushed to. Doesn't shrink the SSE payload further (already addressed by gzip, see below); a narrower fix for who has to wait behind those bytes.
...
- After assembly: method + path parsed from request line. If path is `/api/events`, `registerSSE(conn)` is called and the connection is kept alive indefinitely (see SSE section). Otherwise a `Task` hops to `@MainActor`, response built, `send()` called from network queue.
  **Tracked, not fire-and-forget, added 2026-09-11**: this per-request `Task` is now kept in `inFlightRequestTasks` (a `UUID`-keyed dictionary, `NSLock`-protected via `trackInFlightRequest`/`untrackInFlightRequest`/`snapshotInFlightRequestTasks` — plain `lock()`/`unlock()` calls stay in these small synchronous helpers rather than directly in the async `Task` body, since `NSLock`'s lock/unlock are `NS_SWIFT_UNAVAILABLE_FROM_ASYNC`). `waitForInFlightRequests(timeout:)` gives every currently-tracked task a bounded window to finish (a `withTaskGroup` race against the timeout, same shape as `AppState`'s existing Discord-card-send wait) — called from `AppState`'s SIGTERM handler right before its final `saveConfig()`, so a request still queued behind a busy MainActor (the `await MainActor.run { routeOnMain(...) }` hop above, congested by e.g. an expensive guide-grid rebuild) gets a chance to actually complete and persist instead of being abandoned mid-queue on a `pkill`/`deploy.sh` restart. See `docs/AppState.md`'s matching SIGTERM entry and ISSUES.md's "Web guide Record button fails silently" entry — this closes the gap that entry's "more likely mechanism" theory identified, for the force-quit case specifically (a genuine crash still loses in-flight work; this app only gets a chance to react to SIGTERM in the first place).
...
**Second call site, added 2026-09-11** — `pushFreshTunerCounts()` is now `internal` (was `private`) and also called from `AppState.fetchDeviceStatusUncached`'s hardware-only occupancy-changed branch, right alongside the `broadcastGuideChangeEvent(type: "tuner_occupancy_changed", ...)` call already there. That existing broadcast keeps the grid's `.g-st-inuse` ring in sync, but its payload (`grid`/`sumph`/`tdrop`, see `buildGuideRefreshPayload`) never touches `#dev-bar` — without this second call, the tuner box's own live-count badge sat stale on a hardware-only occupancy change (another Mac/TV, or this app's own Watch Now, locking or freeing the tuner) until a recording start/stop or the periodic guide refresh happened to touch it, even while the grid ring updated immediately.
