...
`guide_refreshed` now falls into case 3 instead of the old fetch-based fallback — the grid the idle loop already rebuilt via `prebuildPageHTML` at guide-refresh time is reused by pushing the identical fragments computed by `broadcastGuideChangeEvent`, so no additional rebuild happens per connected tab. It's broadcast roughly every `AppState.guideRefreshIntervalSeconds(guideHours:divisor:)` (`config.GuideHours` ÷ `config.Guide_refresh_interval_divisor`, default 24h ÷ 8 = 3h nominal) — the actual fire time is jittered within the final hour of that nominal interval (`AppState.jitteredGuideRefreshIntervalSeconds`, re-rolled each cycle) rather than a fixed offset, user-configurable in Settings → Guide — so an idle guide window does a background refresh on roughly that cadence even with no user activity.
