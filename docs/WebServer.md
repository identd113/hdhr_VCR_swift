...
- **Scope row** (`#rm-scope-row`, `em-row` style) — a 2-segment `.rm-type-row` (`#rm-scope`, built by `renderScopeRow(containerId, selected, onSelect)`) — Channel · All — visible only when the Type row's collapsed SeriesID segment is selected (i.e. `_rmType` is `'seriesChannel'`/`'seriesAll'`), re-evaluated on every Type-row click. Selecting a segment sets `_rmType` directly to `'seriesChannel'`/`'seriesAll'`. Mirrors native's Scope Picker (`docs/ShowFormSection.md`).
