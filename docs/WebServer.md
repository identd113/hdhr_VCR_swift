...
**Infomercial dimming:** channels whose guide entries contain any of the confirmed paid-programming SeriesIDs (`C11809220ENAPZK`, `C459763EN3L6D`) get `data-inf="1"` on their `.g-row`. Programs on these rows are always dimmed and unclickable — `applyGenreDim()` adds `.g-prog-dim` to any program whose closest ancestor `.g-row` has `data-inf="1"`, regardless of genre filter state. No UI control; infomercial programs are permanently de-emphasized.
...
| `applyGenreDim()` | Clears all `.g-prog-dim`, then dims any program that fails the genre filter OR is on a `[data-inf="1"]` row — dimmed + unselectable, rows always remain visible |
