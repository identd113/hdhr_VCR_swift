...
- `streamGrowingFile(path:showId:startOffset:conn:)` — sends `200 OK` with `Content-Type: video/mp2t`, `Connection: keep-alive`, no `Content-Length`; if `startOffset > 0`, seeks the `FileHandle` there first (clamped to the file's current size so a stale offset can't seek past EOF, then rounded down to the nearest complete 188-byte TS packet via `alignedToTSPacketBoundary(_:)` — added 2026-09-06 as a regression fix: `startOffset` is usually the recording file's momentary byte size, which has no relation to TS packet framing since curl writes to disk in whatever chunk sizes its own TCP reads land on, so an unaligned seek used to hand a fresh FEED viewer a torn leading packet, whose demuxer then had to scan forward for the next sync byte — the "plays a beat, stalls, fragments of audio" symptom — before it could decode anything at all), then hands off to the pump loop
