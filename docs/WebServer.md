...
- **Type row** — `em-lbl` "Type" label + a compact row of 4 buttons (`.rm-type-btn`, `flex-wrap:wrap` as a safety net but fits on one line at the modal's 480px width), one per `recOpts` entry, labeled with native's exact `ShowState.rawValue` strings — Single · DateTime · SeriesID(Channel) · SeriesID(All) — matching the native wizard's segmented Type picker instead of stacking each option as its own descriptive card. A single description line (`#rm-type-desc`) below the row shows the selected option's longer explanation (e.g. "Record new episodes on this channel"), updating on click. Built and wired up by the shared `renderTypeRow(containerId, descId, selected, onSelect)` helper (also used by the Edit modal) — plain buttons with a `.sel` class for the active one, not radio inputs; the selected value is tracked in the `_rmType` page-level var (read by `confirmRecord()` and the Day-button click handler) rather than a `:checked` DOM query.
