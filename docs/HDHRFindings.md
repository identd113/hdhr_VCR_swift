...
**Forward `Start` limit found — pagination has a firm, low ceiling, not "however far you page," confirmed live 2026-09-13 on device 105404BE.** `Start` itself (independent of `Duration` — even `Duration=1` fails identically) is rejected with `400 Bad Request` once it's more than **exactly 48h ahead of real time** — measured precisely by binary search: `now+48.0h` → `200 OK`, `now+48.05h` → `400`. This mirrors the already-documented **backward** Start limit one section above almost exactly (~48h), suggesting one symmetric ±48h window the server enforces around real time, not a `Duration`-side restriction at all.

Practical consequence for the pagination idea above: chaining `Start` forward doesn't compound indefinitely toward whatever `GuideHours` asks for — it plateaus. Walking `Start` through every value from the first call's own `Duration`-capped edge (~29-31h ahead) up through the 48h wall all returned the **same** `latest EndTime`, ~54.8h ahead of real time (measured directly, not inferred) — so the true forward ceiling reachable via *any* amount of pagination is **~55h (~2.3 days) total, not more**, no matter how many hops are chained. One extra hop past the single-call ~29h cap is genuinely available (worth ~25 more hours); a second or third hop buys nothing further and the next `Start` value simply 400s.

This changes the pagination TODO's cost/benefit: it's a one-time ~25h extension (29h → ~55h), not a general mechanism for reaching an arbitrary `GuideHours` setting like 5 or 7 days. **Confirmed tier-gated, not a platform-wide constant**: `api.hdhomerun.com/api/account?DeviceAuth=<auth>` (see "Endpoints Not Yet Used" below) returned `{"AccountEmail": null, "AccountDeviceIDs": ["105404BE"]}` for this same device at the same test session — no SiliconDust account is linked, i.e. no DVR Service subscription. The ~2.3-day ceiling lining up with `xmltv`'s already-documented "2 days free / 14 days paid" split is very likely the same free-tier limit applied to `guide.php` too, not a `guide.php`-specific quirk. A paid DVR Service account would need to be tested to confirm it actually raises this ceiling (not done — no paid-tier device available).
...
- **Forward window measured live, 2026-08-10 (free tier, no DVR subscription): a hard, uniform ~48 hours.** Per-channel latest `EndTime` across 91 channels: min 48.10h, 25th/median/75th percentile *all exactly* 48.26h ahead of fetch time — a real server-side cutoff, not a soft/varying one. (A handful of channels showed up to 53.26h — one long-running program's `EndTime` spilling a few hours past the common wall, not deeper real coverage.) Confirms the "~2 days free" marketing claim is accurate and is the actual floor for this account, not just an approximation.
  **Re-confirmed 2026-09-13, and now tied directly to `guide.php`'s own ceiling**: fetched live against the same device (105404BE, same free-tier `AccountEmail: null` account) — max `<programme stop>` across 4,776 entries was **1789470000** (~54.7h ahead of fetch time). That is the *exact same epoch* `guide.php`'s own pagination wall capped out at in the same session (see "Forward `Start` limit" finding above) — not just a similar number, the identical absolute timestamp. Strong evidence both endpoints are reading from one shared underlying guide-data horizon gated by the same free-tier limit, rather than two independently-implemented window mechanisms.
...
| `guide.php` `Start` chaining | Pagination to fetch beyond the ~29h single-call cap — confirmed live 2026-09-13 to plateau hard at ~55h total (a firm `Start` ceiling exactly 48h ahead of real time, likely this account's free-tier limit — see Guide API section's "Forward `Start` limit" finding), not a general mechanism for reaching an arbitrary `GuideHours`. Worth one extra ~25h hop; not worth more than that on this account tier. |
