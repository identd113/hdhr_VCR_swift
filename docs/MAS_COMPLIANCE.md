...
| Settings → Web LAN → Terminal Guide's "Open in Terminal" button (`SettingsView.openInTerminal(_:)`) | `NSWorkspace.open(_:withApplicationAt:configuration:)`, not `Process()`/`NSTask` spawning `/usr/bin/open` or Terminal, and not an AppleScript "do script" (which would need the `com.apple.security.automation.apple-events` entitlement plus a user-facing automation-permission prompt). LaunchServices does the actual launch, not this process forking a child, so it needs no new entitlement under a future App Sandbox — same mechanism as Finder's own "New Terminal at Folder" service (which passes a folder URL; this passes the `hdhr_guide` binary's own URL instead, which makes Terminal run it directly — verified live, the same behavior double-clicking a `.command` file gets). Always the app bundle's own `Contents/Helpers/hdhr_guide` (inside the sandbox container), never an arbitrary user-chosen path. |
