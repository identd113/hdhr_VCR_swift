...
Guide_refresh_interval_divisor Int 8    how often AppState.idleLoop's background auto-refresh re-fetches lineup+guide, expressed as GuideHours ÷ this value (default 24h ÷ 8 = 3h nominal) rather than an independent duration — a wider GuideHours window can afford to refresh less often; only 2/4/8 are valid, any other decoded value falls back to 8; nominal interval floored at 30 min (AppState.guideRefreshIntervalSeconds), then the actual fire time is randomized within the final hour of that nominal interval (AppState.jitteredGuideRefreshIntervalSeconds, re-rolled every cycle) rather than always landing at the same offset; "Update Guides Now" bypasses this for an immediate manual refresh
