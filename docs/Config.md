...
**Async save**: `AppState.saveConfig()` (called from ~26 show-mutation sites in `AppState.swift`, essentially every add/edit/delete/pause/resume) snapshots `config`/`shows` synchronously (cheap in-memory copies) then hands the actual remove-backup/copy/atomic-write disk I/O to `ConfigManager.saveAsync(_:)`, which runs it on a private serial background queue instead of inline on `@MainActor` — the previous synchronous write there could stall every web request `WebServer` was mid-hop onto the same actor for. `ConfigManager.save(_:)` itself (the throwing, synchronous form) still exists and is what `saveAsync` calls under the hood — used directly only by `maybeUpgrade`'s v1→v2 migration inside `load()`, a one-time startup path where synchronous, exception-propagating completion is actually wanted. Any caller that must be sure a save has actually reached disk before proceeding (right before the process exits — the SIGTERM handler in `AppState.startup()`, and `teardownForExit()` behind `quit()`/`relaunchForVLC()`) calls `configManager.flushPendingSaves()` right after `saveConfig()`, which blocks until every previously-enqueued save has completed (the queue is serial/FIFO, so this also covers the one just enqueued). Every other call site is fire-and-forget by design — a save that fails there logs `"[Config] Save failed: ..."` and sets `AppState.statusMessage`, same as before this became async.

**Test seam:** `ConfigManager.init(appSupportDir:)` takes an optional directory override; production always passes `nil` and gets the real path above. `AppState.init(configManager:)` accepts an injected `ConfigManager`, defaulting to a real `ConfigManager()` — the app's own startup (`hdhr_VCRApp.swift`) always uses that default. Tests go through `makeTestAppState()` (`Tests/hdhr_VCRTests/TestFixtures.swift`), which points every test's `ConfigManager` at a fresh per-call temp directory. Without this seam, any test exercising a mutating `AppState` path that calls `saveConfig()` (`deleteShow`, `addShow`, `updateShow`, `pauseShow`, `resumeShow`) would silently overwrite the real on-disk config the deployed app uses — this is exactly what would have happened testing `deleteShow`'s side-table cleanup (`Tests/hdhr_VCRTests/AppState/AppStateDeleteShowCleanupTests.swift`) before this seam existed.
...
GuideHours              Int     24      hours of guide to fetch ahead — window size, and also the base that Guide_refresh_interval_divisor divides to derive refresh cadence (see below); clamped to 1...28 (Settings Stepper range, and min(28,...) on decode of an old saved value) — GuideStore.load() makes a single API call and the cloud guide.php endpoint silently truncates any Duration beyond ~29h regardless of what's requested (docs/HDHRFindings.md)
...
Guide_refresh_interval_divisor Int 8    how often AppState.idleLoop's background auto-refresh re-fetches lineup+guide, expressed as GuideHours ÷ this value (default 24h ÷ 8 = 3h nominal) rather than an independent duration — a wider GuideHours window can afford to refresh less often; only 2/4/8 are valid, any other decoded value falls back to 8; the real scheduling function is AppState.effectiveGuideRefreshIntervalSeconds, which floors the nominal interval at 30 min (AppState.guideRefreshIntervalSeconds), randomizes it within the final hour of that interval (AppState.jitteredGuideRefreshIntervalSeconds, re-rolled every cycle), then re-applies the same 30-min floor to that jittered result — raw jitter alone can fall below the floor for any nominal under 90 min, which would defeat its anti-thrash purpose; "Update Guides Now" bypasses this for an immediate manual refresh
...
Write_metadata_sidecar  Bool    false   when true, writes a Kodi-style <episodedetails> .nfo file alongside each recording (same folder/basename, .nfo extension) with the guide's title/season/episode/air-date/synopsis/genre/runtime; season/episode are -1 when EpisodeNumber isn't a full SxxExx tag; write failures are logged, never fail the recording
...
Web_server_enabled      Bool    false   enable NWListener LAN web server (Settings → Sharing → Enable Web LAN)
...
FEED_feature_enabled    Bool    true    master hide switch for Recording FEED, added 2026-09-10, config-file only (no UI sets it) — was `false` until v2.5.0, when it became the release's banner feature. While false: `Virtual_tuner_relay_enabled` below has no effect (the relay never starts regardless of its own value), remote-FEED menu/status-light entries never appear, and the Settings/first-run-wizard FEED sections are skipped entirely. Flip to false directly in the config JSON to hide everything again — see docs/VirtualTunerService.md.
Virtual_tuner_relay_enabled Bool false  Recording FEED — advertise a temporary tuner while recording so another instance can watch it live (Settings → Sharing, see docs/VirtualTunerService.md). Gated by `FEED_feature_enabled` above — has no effect while that master switch is off.
...
Status_light_blink_enabled  Bool false   blink the recording/up-next/remote-FEED-available menu bar status light (6s cycle: 5s lit, 1s off) instead of showing it lit continuously; driven by AppState's own 1Hz timer, independent of the idle loop
