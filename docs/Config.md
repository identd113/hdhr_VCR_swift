...
FEED_feature_enabled    Bool    false   master hide switch for Recording FEED, added 2026-09-10 — config-file only, no UI sets it. While false: `Virtual_tuner_relay_enabled` below has no effect (the relay never starts regardless of its own value), remote-FEED menu/status-light entries never appear, and the Settings/first-run-wizard FEED sections are skipped entirely. Flip to true directly in the config JSON to bring everything back unchanged — see docs/VirtualTunerService.md.
