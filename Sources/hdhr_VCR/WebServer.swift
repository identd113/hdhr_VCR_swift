...
final class WebServer: @unchecked Sendable {
    ...
    // private(set), not private — AppState.startFeedLocalRelay (§1c of the FEED client-side local
    // relay plan) builds a 127.0.0.1 URL against this same bound port, mirroring how
    // beginTranscodeRelay already does from inside WebServer itself.
    private(set) var activePort: Int = 1980
    ...
    private func handleWatchRecording(showId: String, startOffset: Int, conn: NWConnection) {
        ...
        // [self] explicit — this whole chain runs once per request, so strongly holding self for
        // its own brief execution is harmless; only silences the compiler's ImplicitStrongCapture
        // warning about the nested stillActiveCheck closure's own [weak self] (needed there since
        // that closure is retained by streamGrowingFile for the life of a potentially long-lived
        // relay session) differing from this scope's implicit strong capture — same reasoning
        // handleVirtualTunerStream already documents for its own [state] capture below.
    ...
    // `showId`/`startOffset` build the inner URL directly rather than going through the opaque
    // session registry `/api/feed-local-relay` uses: there's no separate "session" here to
    // register/unregister against a window's own lifecycle, just a deterministic loopback URL
    // derived straight from this request's own params. `startOffset` is passed through unchanged
    // (the true live edge, zero backlog) — a same-day attempt to also start this connection a few
    // seconds behind live (so there'd be a real backlog to read, reducing streamGrowingFile's own
    // live-edge poll frequency) was tried and reverted: the resulting fast backlog-drain delivered
    // ~1.5MB to the pacer below within milliseconds of connecting, poisoning its own rate estimate
    // (a plain cumulative average) into treating that burst as the sustainable rate, defeating the
    // smoothing entirely. Zero backlog here matches the real remote FEED case this same pacer was
    ...
    private func handleVirtualTunerStream(channel: String, deviceId: String?, durationSeconds: Int?, transcode: String?, conn: NWConnection) {
        ...
        // differing from these outer scopes' implicit strong one; no behavior change. `self` added
        // to this same capture list for the same reason, once the FEED local-relay stillActiveCheck
        // closure (also [weak self], same lifetime as onStreamEnded) started passing through here too.
        Task { @MainActor [state, self] in
    ...
    private func beginTranscodeRelay(showId: String, profile: String, conn: NWConnection, durationSeconds: Int? = nil, startOffset: Int = 0) {
        ...
        // 2026-09-13: this source read is never watched directly by a human — only the transcode's
        // own real-time *output* (paced separately, see TranscodeProxyDelegate/
        // RelayProxyDelegateBase) is ever actually seen by a viewer — so smoothing curl's own bursty
        // on-disk writes before they reach libvlc's encoder matters here exactly as much as it
        // already did for a real remote FEED viewer's own playback (issues_resolved.md's "VLC-side
        // FEED playback stalls" entry): confirmed live via repeated `late buffer for mux input`
        // warnings under 2 concurrent transcode viewers. handleTranscodeSourceRelay reuses the exact
        // same proven pacer (FeedRelayProxyDelegate) against this app's own loopback
        // /api/watch-recording, so libvlc's transcode source connection gets a steady real-time
        // trickle instead of a raw burst.
        //
        // Starting at `startOffset` (the true live edge, zero backlog by construction), not behind
        // it: a same-day deliberate "start a few seconds behind live so there's a real backlog to
        // read" attempt was tried and reverted — the resulting fast backlog-drain delivered ~1.5MB
        // to the pacer within milliseconds of connecting, which poisoned its own rate estimate
        // (a plain cumulative average, bytesReceived/elapsed since connect) into believing that
        // burst *was* the sustainable throughput, defeating the smoothing entirely. The real remote
        // FEED case this pacer was built for never hits this, since a fresh FEED connection always
        // starts at zero backlog too — matching that same shape here, rather than fighting the
        // pacer's own math to tolerate a burst it was never designed to see, is what actually keeps
        // delivery paced like a standard real-time stream from the first byte.
        let sourceURL = "http://127.0.0.1:\(activePort)/api/transcode-source-relay?show=\(showId)&start=\(startOffset)"
    ...
    private func handleGrowingFileChunk(_ chunk: Data, handle: FileHandle, showId: String, conn: NWConnection, path: String,
        ...
        guard !chunk.isEmpty else {
            ...
            Task { @MainActor [weak self] in
                guard let self else { return }
                let stillRecording = stillActiveCheck()
                if stillRecording {
                    self.queue.asyncAfter(deadline: .now() + Self.liveEdgePollInterval) { [weak self] in
                        self?.pumpGrowingFile(handle: handle, showId: showId, conn: conn, path: path,
                                               bytesSent: bytesSent, waitStreak: waitStreak + 1, waitStartedAt: startedAt,
