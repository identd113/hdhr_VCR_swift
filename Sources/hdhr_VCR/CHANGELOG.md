...
**Updated**
- **The default "Min free disk" setting is now 30 GB (was 10 GB)** — recordings are skipped below this much free space on the save drive, and it's still adjustable in Settings → Recording. Raised because 10 GB was cutting it too close on modern large drives. This is separate from, and doesn't change, the existing rule that also skips recording once a drive is over 93% full regardless of how much space that percentage represents.
...
- Gave the channel-signal-history store and the channel-icon disk cache a test-only way to point at a temp directory instead of the real one (same pattern the app's config loader already used), then added real tests against both — signal bucket thresholds/sample retention/persistence round-trip, and icon-cache disk hits/eviction/collision-safety. Coverage on those two files went from ~2-13% to ~86-98%. No user-visible change.
- Reorganized the test suite's 31 files (previously all flat in one folder) into subfolders by what they cover — Models, Network, Recording, AppState, Caches, WebServer, Views — mirroring the app's own architecture. No behavior change; verified the full suite (297 tests) still passes, including the snapshot tests (whose reference-image lookup is relative to the test file's own location, so it had to move together with them) and the opt-in real-window navigation suite.
- Added real test coverage for the recording-scheduling engine (when a recording actually starts, stops, retries, and reschedules) — previously completely untested. `AppState.swift` coverage: 20.34% → 39.03%. Surfaced the "Min free disk" default being too low for modern drives, addressed above under Updated.
