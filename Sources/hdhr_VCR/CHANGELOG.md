...
- Added real test coverage for the recording-scheduling engine (when a recording actually starts, stops, retries, and reschedules) — previously completely untested. `AppState.swift` coverage: 20.34% → 39.03%. No behavior change for the real app. Surfaced (but did not fix — see `ISSUES.md`) a real bug while doing this: recordings can silently refuse to start on a drive that's over 93% full even when tens of GB are genuinely free, because the disk-space guard checks percentage-full in addition to (not instead of) the user-adjustable "minimum free space" setting.
