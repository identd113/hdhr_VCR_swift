...
**Minimum OS**: macOS 14.0 (set in `Package.swift` as the string literal `"14.0"` — the enum form triggers a SourceKit false diagnostic on macOS 26 Beta toolchains, so the string form is used instead). `keyframeAnimator` (macOS 14+) is used in `StarburstBadge`. `onScrollGeometryChange` (macOS 15+) is used in `CableGuideView` scroll sync but has a PreferenceKey fallback for macOS 14 via `View.onScrollOffset` in `CompatibilityHelpers.swift`.
...
The channel label column is **not** part of the scroll view — it stays pinned on the left while scrolling right. Vertical scroll is synchronized from the inner `ScrollView` to the fixed column via `.onScrollOffset(coordinateSpaceName:)` (a `CompatibilityHelpers` extension: uses native `onScrollGeometryChange` on macOS 15+, falls back to a `PreferenceKey + GeometryReader` approach on macOS 14). 1-pt threshold + `Transaction.disablesAnimations` prevent jitter; result applied as `.offset(y: -channelScrollOffset)` inside `.clipped()`.
