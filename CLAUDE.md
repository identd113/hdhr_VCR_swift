...
**Web guide managed markers are tuner-scoped** — both `seriesChannel` and `seriesAll` shows (blue `.g-st-sched` status ring + ⏱ badge, see `docs/WebServer.md`'s "Status ring + badge") use `"deviceId:SeriesID"` / `"deviceId:title"` keys in `ManagedGuideMatcher` (`seriesKeys`/`seriesTitles`); both only appear on the assigned tuner (`hdhr_record`) — a series show can never match/record on more than one tuner at once. The only difference between the two states is channel scope within that one tuner: `seriesChannel` locks scheduling to the one channel it was added from; `seriesAll` follows the series across any channel on that same tuner (reruns/affiliates often shift channels) — enforced in `AppState.resolveSeriesAir`/`nextGuideEpisode`/`scheduleNextAir` via a `channelNum` filter that's `nil` only for `seriesAll`, with `deviceId` always pinned to the show's own tuner for both. `dateTime` slot keys include weekday (`"device:channel:Weekday:HH:MM"`) so a Wednesday-only show doesn't flag Friday reruns. When **Skip already-recorded episodes** is on (with Series subfolders), a managed block whose `SxxExx` is already on disk shows the slate `.g-st-skip` ring + ⏭ badge (`data-skip=1`) *instead of* the blue `.g-st-sched` one — computed in `buildGuideGridHTML` via `AppState.recordedEpisodeTags(...)`, one scan per managed series per build.
