...
**Guide page CSS/JS/HTML live in `Resources/guide.css`/`guide.js`/`guide-shell.html`** — real files, not Swift string literals, loaded once via `Bundle.main` (`WebServer.swift`'s `cachedGuideCSS`/`cachedGuideJS`/`cachedGuideShellHTML`) and stitched together in `buildHTML()` via `fillTemplate(_:_:)`, which substitutes `{{TOKEN}}` placeholders for the ~15 dynamic values (tuner JSON, guide window, config flags, etc). Normal JS/CSS escaping rules apply — no more Swift-string double-escaping — and `node --check Resources/guide.js` validates JS syntax directly. `deploy.sh`/`deploy_release.sh` must copy all three into `Contents/Resources/` alongside the other bundled resources; a `#if DEBUG` fallback in `templateURL(_:_:)` lets a plain `swift build` + direct binary run still find them via a source-relative path when there's no `.app` bundle. **No auth beyond LAN-subnet matching** on any endpoint — validate inputs defensively (paths, IDs, ranges) on any new mutating route rather than trusting the caller.
