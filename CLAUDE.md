...
- **Tuner picker + Refresh button** in the header: switch between tuners (hidden when only 1) or force a guide reload. Changing the tuner invalidates its cache and reloads via `.task(id: taskId)` where `taskId` combines `selectedDevice?.DeviceID` and a `refreshToken UUID`.
- Uses `state.guideStore` cache; fetches fresh if absent or after Refresh
- Show blocks are colour-coded by SeriesID/title hash (consistent colour per show)
- Currently-airing shows have full opacity + red dot; future shows are dimmed
...
- Red "now" line runs through all rows; view auto-scrolls to it on open
- Time header row is pinned (sticky) while scrolling vertically
- **Show summary panel** (120 px fixed, between header and grid): when a show is selected shows poster image (AsyncImage), title, episode number · episode title, synopsis (3 lines), channel + time range. Shows a placeholder when nothing is selected.
- Window is 960×700 for this step, 560×540 for others
