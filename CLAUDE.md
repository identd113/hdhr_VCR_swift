...
| `hdhrVCRplus.app/Contents/Helpers/hdhr_guide` | **Bundled, not in `tools/`** — a separate SPM executable target (`Sources/hdhr_guide/`), copied and signed into the app bundle by both deploy scripts. Full-screen terminal client for the guide: browse channels/times, schedule a recording, all via the existing LAN web server's JSON API (`/api/guide.json` + `POST /api/record`). See `docs/TUIGuide.md`. |
