...
**Minimum OS**: macOS 15.0 (set in `Package.swift` as the string literal `"15.0"` — the enum form triggers a SourceKit false diagnostic on macOS 26 Beta toolchains, so the string form is used instead). `keyframeAnimator` (macOS 14+) is used in `StarburstBadge`. `onScrollGeometryChange` (macOS 15+) is used directly in `CableGuideView` scroll sync — no fallback needed since 15.0 is the floor.
...
- [SettingsView](docs/SettingsView.md) — settings window, draft/save pattern, WindowCloseInterceptor
