...
**Transcode capability gate** — `HDHRDevice.supportsTranscode` (`Models.swift`, `ModelNumber` starting with `"HDTC"` — nil/unknown ModelNumber, e.g. a UDP-only-discovered device, conservatively means `false`) is checked once, right before `AppState.startRecording` calls `recordingManager.start(...)`: `effectiveTranscode = device.supportsTranscode ? show.show_transcode : "none"`. This is the *only* place the override happens — `show.show_transcode` itself is never mutated by it, so the Transcode picker (native `ShowFormSection`, web `#rm-transcode`/`#em-transcode`) still shows the user's actual saved choice even on an unsupported tuner. A new recording call site must reapply this same gate itself; it will not inherit it by calling through `startRecording`'s existing plumbing unless it *is* `startRecording`. Both native (`ShowFormSection`) and the web guide's Record modal (`#rm-no-transcode`, gated on `tuners[devId].nt` — see `docs/WebServer.md`) show a proactive warning when the picked profile would be overridden — informational only, the real enforcement is server-side and reruns unconditionally regardless of what any UI warned about. See `docs/HDHRFindings.md`'s "Transcode capability" section for why the real per-device capability field (a `Transcode` JSON field) is deliberately never queried — it only exists on a CGNAT-unsafe cloud endpoint.
